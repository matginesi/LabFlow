#!/usr/bin/env python3
"""Guard the single canonical ExperimentData + Operation runtime state."""
...
errors=[]
for m in ('function ensureExperiment(reason)','function touch(scope)','function setExperiment(exp, rawArchive)','function startOperationRun(record)','function endOperationRun(status)'):
...
if 'derived: { operations: {}, chat: { conversation: [] } }' not in model: errors.append('derived must contain only operation history + chat')
if 'exp.manifest = manifest' not in importer: errors.append('importer does not write canonical manifest')
if 'LF.DataModel.getExperiment()' not in ctx: errors.append('OPERATION context does not resolve canonical experiment')
