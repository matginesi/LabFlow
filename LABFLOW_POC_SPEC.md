...
A separate browser-internal **Knowledge Base** manages sourced materials, formulations, processes, device stacks and LabFlow product guides. Its bundled `knowledge-base/library.json` is merged automatically into the IndexedDB library at startup, so new bundled records appear without folder authorization while same-ID local edits and personal records remain authoritative. The library is immediately manageable and is not scientific experiment state. Retrieval is an optional enrichment that the researcher can enable or disable: selected AI Actions and the Assistant work normally without RAG, and receive bounded relevant records only while retrieval is active, healthy and useful. Scientific Actions exclude product-guide records; the Assistant may retrieve them for questions about LabFlow. Design never asks the researcher to copy records first; every used scientific record ID remains on the reviewable proposal and the normal deterministic fill-only gate protects source/researcher values. Its own page searches and edits records, merges JSON imports and exports backups. A researcher-selected `library.json` folder is optional synchronization/backup only. **Reset session** never modifies the internal library, retrieval preference or optional folder mirror.

`design.infer` is optional and receives only the currently selected experiment, its explicit missing fields and linked evidence. When Knowledge retrieval is enabled and healthy, it may also receive an automatically ranked bounded set of relevant local records; RAG is never required. Local records remain non-experimental knowledge context unless RAW evidence independently supports them. Used records are identified by stable `knowledge_refs`; quantitative library values remain paper-specific candidates. If retrieval is disabled, unavailable, empty or irrelevant, the model still returns a conservative qualitative candidate marked as unsourced model knowledge, with no fabricated record IDs and confidence capped at 35%; unsupported quantities remain unknown. The internal Action produces a validated proposal; the explicit **Complete missing with AI** command immediately merges its locally applicable values through the deterministic fill-only gate without overwriting user-confirmed fields. A reason-only or otherwise non-applicable response fails validation and is never reported as completed. Each proposal displays a 0–100 evidence-weighted reliability estimate across its decisions; this is explicitly an estimated review aid, not observed ground-truth accuracy or a replacement for validation.
