...
`design.infer` is optional and receives only the currently selected experiment, its explicit missing fields, selected sample identities and linked Design evidence. It may also receive a small automatically ranked set of scientific Knowledge Base records when local lookup finds something useful. A miss is normal and model inference continues without an artificial confidence penalty. Every proposed field keeps source, confidence and deterministic auto-apply safety separate. Exact model-inferred recipe/process quantities stay visible for researcher review unless supported by experiment evidence or a valid KB record; high-confidence qualitative values may be applied automatically. Identifiers such as `N2`, `SnO2`, `C60` and `2PACz` are not quantities merely because they contain digits. **Complete missing with AI** remains fill-only and never overwrites source/user-confirmed values. Review-only and explicit unknown results complete successfully. The UI prioritizes applied/review/unresolved counts and uses mean confidence across proposed fields only as a secondary indicator.
