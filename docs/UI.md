...
Lab Cabinet is a browser-local library of reusable scientific recipes and definitions, not inventory or LIMS. The page must explain the value before exposing management controls: **Save → Reuse → the experiment keeps its own copy**. With an experiment open, quick capture exposes formulations, device stacks and process recipes from the current Design. The canonical UI is purpose/capture above one searchable/type-filtered tile shelf and a full-width selected-resource editor, with portable backup behind progressive disclosure. Do not use a table-like grey header slab, permanent left/right master-detail split or a dense tab strip for resource types. The shelf is 3 columns on wide canvases, 2 on tablet and 1 on phone; the library toolbar and editor fields reflow before typography shrinks. Resources are copied into Design as detached snapshots. Incomplete resources may be edited but are not valid matches/apply targets.
