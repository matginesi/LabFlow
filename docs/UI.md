...
Connection keeps provider, exact model, endpoint and credential status visible; request timeouts, output caps, streaming and thinking overrides stay under progressive disclosure. Detect and Save & test are explicit, never run on page load, send no experiment data, and report their operation result through the canonical Message Totem rather than an inline provider-status clone or Action Totem. Explicit-save sections show saved versus unsaved state, validation sits beside the relevant field, and credentials remain masked and browser-local. NOMAD remains configuration-only: saving performs no request and cannot imply upload or publication. Local-capable providers use their normal product names (no `(local)` suffix). Connection shows one compact LOOPBACK/LAN/REMOTE summary and keeps network diagnostics behind disclosure; do not place long provider/network prose permanently in the form. A loopback endpoint is explicitly described as same-device only, while LAN hostnames/private IPs are supported when the server bind, browser Local Network permission and CORS allow them. Detect reads the visible endpoint/key and, for catalogue providers, can populate an initially empty model selector from that same live catalogue before requiring a real Chat Completions probe; catalogue metadata alone never counts as connectivity success.
