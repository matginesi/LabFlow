...
When the model is cached, the same setup Totem covers the runtime/cache/adapter check when it is not instantaneous: after a short grace period (about 300 ms, applied to the initial cache/runtime check as well as the load/warm-up) it appears as a loading Totem with a blurred backdrop, so a fast start stays uninterrupted while a slow start is explicit instead of leaving the interface apparently idle. While the application scripts load, `index.html` shows a first-paint boot card instead of an empty workspace.
