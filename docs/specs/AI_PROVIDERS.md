...
LabFlow uses the OpenAI-compatible Chat Completions shape. Every provider uses the endpoint configured by the user. Local/LAN endpoints are browser-direct; hosted endpoints are direct when CORS permits or use the explicit same-origin local relay when LabFlow is served by `tools/labflow_server.py`:
...
- Hosted endpoints must permit browser CORS for direct use. When LabFlow is served by `tools/labflow_server.py`, the transport may instead use its explicit same-origin allow-listed relay; on GitHub Pages or another plain static host no relay exists. An HTTP response such as 401/403/429/5xx is always preserved as a provider response, never mislabeled as CORS.
...
- NVIDIA NIM therefore resolves the default hosted endpoint to `https://integrate.api.nvidia.com/v1/models`, but a self-hosted/custom NIM endpoint resolves to its own `/v1/models`. The same user-supplied bearer key is used for catalogue and inference. **Detect** presents returned model IDs in the shared select only after a real catalogue request succeeds. Z.AI uses the same **Detect** UI with a documented built-in catalogue snapshot and built-in capability metadata (default `glm-4.7-flash`); LabFlow does not invent an undocumented Z.AI `/models` dependency.
...
Discovery is cached by provider, endpoint and model. Opening Settings, editing provider fields and running an Action never contacts a provider for metadata. Every provider uses the same explicit **Detect** contract: it reads the provider, endpoint/base URL, model and API key from the **visible form values**, never from an older saved fallback; a missing required key is an immediate error; catalogue providers must return a non-empty catalogue; and every Detect finishes with a real bounded Chat Completions probe. A catalogue or capability fallback can never turn an unreachable provider into a green result. Providers that expose useful catalogues read them; Z.AI uses its documented built-in catalogue snapshot but still must pass the real inference probe. Without detected or built-in metadata, normal Actions use their bounded output contract and a conservative unknown-capability fallback. **Save & test connection** probes the exact visible provider/endpoint/model/key tuple and saves it only after that probe succeeds. Detect and Save & test use the canonical **Message Totem** for operation success/failure; Settings has no parallel inline provider-status component. Exact output limits are preferred when Detect has established them, and a context window is not mislabeled as an output limit.
...
**Save & test connection** validates and probes the exact visible provider, model, endpoint and provider-scoped key first, then persists them only after a successful probe. The probe is one minimal non-streaming request through the unified transport (single provider attempt, no semantic retry). It uses `Reply only with OK.`, a small output ceiling, a shorter timeout and an explicit thinking-off field when the provider declares one. It performs no capability/model discovery and does not change the selected model. HTTP 429 and known provider limit/quota codes produce a reachable-but-limited result with provider code/message and `Retry-After` when available; they create no local cooldown and do not block a later explicit request. The provider output includes the model reply plus measured timing, token usage or a clearly marked estimate, payload size, finish reason and request ID. It does not send experiment data.

### Unified provider transport

`transport.js` shares one header/auth builder (`providerAuthHeaders`), one URL resolver (`resolveChatUrl`/`resolveModelsUrl`), one metadata fetch (`metadataFetch`/`fetchJson`) and one error classifier (`parseProviderError`/`limitInfo`) for connection test, capability Detect and model listing. Local/LAN endpoints are fetched directly. For allow-listed hosted providers, the transport uses the same-origin relay exposed by `tools/labflow_server.py` when that server is actually serving the page; otherwise it attempts direct browser fetch and reports CORS/network failure honestly. `listModels`, `resolveModelCapabilities`, `benchmarkTokensPerSecond` and `testConnection` are strategies inside the same transport contract rather than separate provider UI workflows. `send()` performs exactly one HTTP request and surfaces provider throttles unchanged; semantic Action retries, when declared, remain a separate runner concern.
...
### Browser CORS and the optional local relay

LabFlow remains a static vanilla-JS frontend, but not every hosted AI API permits cross-origin calls from arbitrary browser pages. `tools/labflow_server.py` is a small Python-standard-library static server with an allow-listed same-origin relay for Z.AI, OpenRouter, NVIDIA NIM, OpenAI and Gemini. It forwards only GET/POST requests to the official configured host for the selected provider and never acts as a general proxy. The relay is selected automatically when available and is visible in logs as `transport=relay`; provider HTTP 401/403/429/5xx responses are preserved rather than converted into false network success.

A GitHub Pages deployment has no such same-origin Python relay. Direct browser operation there therefore depends on each provider's CORS policy. Local/LAN endpoints remain direct and depend on the local server bind, CORS and the browser's Local Network Access policy.
