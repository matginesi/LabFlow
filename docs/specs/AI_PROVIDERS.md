...
LabFlow uses the OpenAI-compatible Chat Completions shape. Every provider uses the endpoint configured by the user. The browser sends the request directly to that configured endpoint:
...
- Hosted endpoints normally must permit browser CORS for the LabFlow origin. An HTTP response such as 401/403/429/5xx is always preserved as a provider response, while a browser-level CORS/network failure is reported separately.
- Hosted Z.AI and NVIDIA NIM are the built-in CORS exceptions. Their presets are `http://127.0.0.1:8099/zai/v1` and `http://127.0.0.1:8099/nvidia/v1`, served by `tools/provider_relay.py`. The helper has an explicit route allowlist, stores no credentials, owns no scientific state and is not a general-purpose provider proxy. Browser-compatible custom/self-hosted endpoints may be configured directly.
...
- NVIDIA NIM derives `/models` from the visible Settings endpoint like every other catalogue provider. The built-in preset therefore probes `http://127.0.0.1:8099/nvidia/v1/models`; the bundled relay forwards that request to the official hosted NVIDIA `/v1/models`. A self-hosted/custom NIM endpoint resolves to its own `/v1/models`. The same user-supplied bearer key is used for catalogue and inference and is forwarded only for the current request. When the browser can read the catalogue, **Detect** presents the returned model IDs in the shared select. If only catalogue access is unavailable but an exact model ID is already configured, Detect may continue with that configured model; it is successful only if the subsequent live Chat Completions probe succeeds. Z.AI uses the same **Detect** UI with a documented built-in catalogue snapshot and built-in capability metadata (default `glm-4.7-flash`); LabFlow does not invent an undocumented Z.AI `/models` dependency.
...
### Browser CORS and configured-endpoint transport

LabFlow is intentionally a static vanilla-JS frontend. The browser contacts the endpoint shown in Settings; no hidden provider fallback or general-purpose backend exists. Local/LAN access therefore requires the server to bind to a reachable interface, allow the LabFlow origin with CORS, and satisfy the browser Local Network Access policy. Hosted APIs normally must expose browser-compatible CORS for the LabFlow origin. Z.AI and NVIDIA hosted NIM are handled by the explicit local relay presets described above because the deployed GitHub Pages browser cannot read their hosted responses directly. If the browser cannot reach the configured endpoint or otherwise exposes no HTTP response, Detect/Save & test fail explicitly and Logs classify the failure as browser/network rather than authentication.
