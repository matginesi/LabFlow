...
module.exports=function(t,LF,ctx){
  ...
  t['page route order is one shared source for workflow and in-page navigation']=function(){
    ...
    if(!/\['diagnostics'\s*,\s*'Diagnostics'\s*,\s*'Support tools'\]/.test(settings)||!/\['ui-kit'\s*,\s*'UI Kit'\s*,\s*'Design reference'\]/.test(settings))throw new Error('Diagnostics and UI Kit must live under Settings → Advanced');
  };
