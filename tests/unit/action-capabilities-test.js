...
module.exports=function(t,LF,ctx){
  ...
  t['public catalog is global while route only changes recommendation']=function(){
    const e=exp(false);LF.State={state:{experiment:e,route:'experiment-export',selectedDesignDeviceId:'d1',boxPlot:{groups:['G1']}}};
    const exportIds=LF.ActionCapabilities.catalog().map(function(x){return x.id;}).sort();
    LF.State.state.route='experiment-design';
  ...
  t['semantic resolution is unavailable before execution when no ambiguity exists']=function(){
    const e=exp(false);LF.State={state:{experiment:e,route:'experiment-export',selectedDesignDeviceId:'d1',boxPlot:{groups:[]}}};
  ...
  t['semantic resolution becomes available without changing page when ambiguity appears']=function(){
    const e=exp(true);LF.State={state:{experiment:e,route:'experiment-export',selectedDesignDeviceId:'d1',boxPlot:{groups:[]}}};
    const a=LF.ActionCapabilities.evaluate('dataset.resolve-ambiguities');
  ...
  t['Design target binding lives in capability service and follows selected experiment']=function(){
    const e=exp(false);e.design.devices.push({id:'d2',solutionIds:[],stack:[],process:{}});LF.State={state:{experiment:e,route:'experiment-results',selectedDesignDeviceId:'d2',boxPlot:{groups:[]}}};
  ...
  t['Results comparison bindings are manifest-driven and guard the same resolved selection']=function(){
    const e=exp(false);e.measurements=[{id:'m1',group:'G1'},{id:'m2',group:'G2'}];LF.State={state:{experiment:e,route:'experiment-export',selectedDesignDeviceId:'d1',boxPlot:{groups:['G1','G2'],metric:'voc',direction:'rv',eligibleOnly:false}}};
    ...
    LF.State.state.boxPlot.groups=['G1'];const b=LF.ActionCapabilities.evaluate('results.compare');assert(b.available,false,'same guard sees updated binding');assert(b.reason,'Select at least two Results groups to compare.','exact blocker');
  };
  ...
  t['slash commands resolve from Action manifests rather than Assistant aliases']=function(){
    LF.State={state:{experiment:exp(true),route:'experiment-import',selectedDesignDeviceId:'d1',boxPlot:{groups:[]}}};
