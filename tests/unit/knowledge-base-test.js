...
module.exports=function(t,LF){
  ...
  t['Bundled Knowledge Base is populated from the JSONL source bundle']=function(){
    ...
    if(stats.bundled<117)throw new Error('Expected expanded scientific + documentation KB bundle, got '+stats.bundled);
    ...
    const designCompact=LF.KnowledgeBase.compactForDesign(LF.KnowledgeBase.get('architecture.pin-ito-2pacz-perovskite-c60-bcp-ag'));
    if(!designCompact||!designCompact.design_hint)throw new Error('Design compact view must preserve structured hints');
    if(/sources|citation|doi|url/i.test(JSON.stringify(designCompact)))throw new Error('Design compact view leaked bibliography metadata');
    const assistantCompact=LF.KnowledgeBase.compactForAssistant(LF.KnowledgeBase.get('material.peai'));
    if(!assistantCompact||!(assistantCompact.sources||[]).length)throw new Error('Assistant compact view must retain traceable sources');
  };
