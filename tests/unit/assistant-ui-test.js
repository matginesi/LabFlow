...
module.exports=function(t){
  ...
  t['02 a deterministic activity status is present in the initial requesting state']=function(){assert(js.includes('pending?transientHtml(m)'),true,'requesting state renders transient');assert(js.includes('chat-thinking-dot'),true,'activity indicator exists');assert(js.includes("Thinking · "),true,'elapsed status is updated while waiting');};
  ...
  t['07 thinking never creates a second bubble']=function(){assert(js.includes('class="chat-thinking"'),false,'removed thinking bubble stays absent');assert(js.includes("if(reasoning)run.message.reasoning=reasoning"),true,'reasoning stays on same message');};
  ...
  t['12 thinking has no nested scroll container']=function(){assert(css.includes('.chat-thinking {'),false,'removed scrolling thinking component stays absent');assert(/\.chat-transient\s*\{[^}]*overflow/s.test(css),false,'transient does not scroll');};
  ...
  t['15 Action results are compact system events']=function(){assert(js.includes("role:'system'"),true,'Action uses system role');assert(js.includes('chat-event'),true,'compact event markup');assert(/\.chat-event\s*\{/.test(css),true,'event styling exists');};
  ...
  t['17 Assistant and context contracts stay connected']=function(){assert(context.includes("profile==='assistant'"),true,'Assistant context pack preserved');assert(context.includes("m.role==='user'||m.role==='assistant'"),true,'events excluded from memory');};
  ...
  t['20 activity cleanup is structural and has a bounded request watchdog']=function(){assert(js.includes('finally{'),true,'finally cleanup');assert(js.includes('clearInterval(run.clock)'),true,'clock cleanup');assert(js.includes('window.clearTimeout(run.watchdog)'),true,'watchdog cleanup');assert(js.includes('ASSISTANT_TIMEOUT'),true,'hung provider cannot leave Assistant active forever');};
