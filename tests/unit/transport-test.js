...
module.exports = function (t, LF) {
  ...
  t['resolveChatUrl normalizes bare and double-suffixed endpoints'] = function () {
    assert(AI.resolveChatUrl('https://provider.example/v1'), 'https://provider.example/v1/chat/completions', 'append');
    assert(AI.resolveChatUrl('https://provider.example/v1/chat/completions'), 'https://provider.example/v1/chat/completions', 'already complete');
  ...
  t['SSE DONE terminates immediately even when provider keeps the HTTP stream open'] = async function () {
    ...
    global.fetch=async function(){const body=new ReadableStream({start:function(controller){controller.enqueue(encoder.encode('data: {\"id\":\"done-test\",\"model\":\"test-model\",\"choices\":[{\"delta\":{\"content\":\"ready\"}}]}\n\ndata: {\"choices\":[{\"finish_reason\":\"stop\"}]}\n\ndata: [DONE]\n\n'));closeTimer=setTimeout(function(){producerClosed=true;try{controller.close();}catch(_){ }},500);},cancel:function(){streamCancelled=true;if(closeTimer)clearTimeout(closeTimer);}});return{ok:true,status:200,statusText:'OK',headers:new Headers({'content-type':'text/event-stream'}),body:body};};
    LF.Storage={getAiSettings:function(){return{provider:'custom',endpoint:'https://provider.example/v1',model:'test-model',inactivityTimeoutMs:60000,streaming:true};},getApiKey:function(){return'key';}};
    LF.AIProviders={custom:{id:'custom',keyRequired:true,tokenParam:'max_tokens',supportsStreaming:true,supportsTemperature:true}};
  ...
  t['buildRequest produces a valid Chat Completions body'] = function () {
    LF.Storage = {
      getAiSettings: function () { return { provider: 'custom', endpoint: 'https://provider.example/v1', model: 'test-model', temperature: 0.4, maxTokens: 2200, inactivityTimeoutMs: 90000, streaming: true }; },
      getApiKey: function () { return 'test-key'; }
    };
    LF.AIProviders = { custom: { keyRequired: true, tokenParam: 'max_tokens', supportsStreaming: true, supportsTemperature: true } };
    const spec = AI.buildRequest({ provider: 'custom', messages: [{ role: 'user', content: 'hi' }], stream: true, maxTokens: 512 });
    assert(spec.url, 'https://provider.example/v1/chat/completions', 'url');
    assert(spec.headers.Authorization, 'Bearer test-key', 'auth header');
    assert(spec.body.model, 'test-model', 'model');
  ...
  t['provider default deadline prevents unbounded requests when an Action omits one'] = function () {
    LF.Storage={getAiSettings:function(){return{provider:'custom',endpoint:'https://provider.example/v1',model:'test-model',inactivityTimeoutMs:90000,streaming:true};},getApiKey:function(){return'key';}};
    LF.AIProviders={custom:{id:'custom',keyRequired:true,tokenParam:'max_tokens',supportsStreaming:true,requestDeadlineMs:180000}};
  ...
  t['network errors retain the provider from the current request rather than saved Settings']=async function(){
    ...
    LF.Storage={getAiSettings:function(){return{provider:'openai',endpoint:'https://saved.invalid/v1',model:'saved',inactivityTimeoutMs:15000,streaming:false};},getApiKey:function(){return'saved-key';}};
  ...
  t['llama.cpp reasoning-only bounded probe is reachable, not a connection failure'] = async function () {
    ...
    LF.PromptRegistry={promptText:function(){return'Reply OK';}};
    LF.Storage={getAiSettings:function(){return{provider:'llamacpp',endpoint:'http://127.0.0.1:8080/v1',model:'local-model',maxTokens:4096,inactivityTimeoutMs:60000,streaming:false,thinkingMode:'auto'};},getApiKey:function(){return'';}};
    LF.AIProviders={llamacpp:{id:'llamacpp',keyRequired:false,tokenParam:'max_tokens',supportsStreaming:true,supportsTemperature:true,connectionTestTimeoutMs:15000,connectionTestMaxTokens:64,connectionTestAcceptReasoningOnly:true,safeThinkingOverrideWhenUnknown:true,thinkingModes:{off:{reasoning_effort:'none',chat_template_kwargs:{enable_thinking:false,reasoning_effort:'none'}}}}};
  ...
  t['llama.cpp reasoning-off Action can override unknown model reasoning metadata safely'] = function () {
    const provider={safeThinkingOverrideWhenUnknown:true,thinkingModes:{off:{reasoning_effort:'none',chat_template_kwargs:{enable_thinking:false,reasoning_effort:'none'}}}};
    ...
    assert(body.reasoning_effort,'none','reasoning effort disabled');assert(body.chat_template_kwargs.reasoning_effort,'none','template reasoning effort disabled');assert(body.chat_template_kwargs.enable_thinking,false,'template thinking disabled');
  };


  t['reasoning-off prompt guard adds final-only instruction only when explicitly requested'] = function () {
    LF.Storage={getAiSettings:function(){return{provider:'llamacpp',endpoint:'http://127.0.0.1:8080/v1',model:'local-model',streaming:true,thinkingMode:'auto'};},getApiKey:function(){return'';}};
    LF.AIProviders={llamacpp:{id:'llamacpp',keyRequired:false,tokenParam:'max_tokens',supportsStreaming:true,supportsTemperature:true,thinkingPromptGuard:true,thinkingModes:{off:{reasoning_effort:'none'}}}};
    try{const guarded=AI.buildRequest({messages:[{role:'user',content:'return JSON'}],stream:true,thinkingMode:'off',guardThinking:true}),plain=AI.buildRequest({messages:[{role:'user',content:'return JSON'}],stream:true,thinkingMode:'off',guardThinking:false});assert(guarded.body.messages[0].role,'system','guard becomes system instruction');if(guarded.body.messages[0].content.indexOf('Produce only the final requested answer')<0)throw new Error('final-only guard missing');assert(plain.body.messages[0].role,'user','guard is not injected into direct requests unless requested');}
    finally{delete LF.Storage;delete LF.AIProviders;}
  };

  t['llama.cpp runtime reasoning control stops a template that ignores thinking-off'] = async function () {
    const oldFetch=global.fetch;let controlBody=null,mainBody=null,calls=0;const encoder=new TextEncoder();
    global.fetch=async function(url,opts){calls++;const target=String(url);if(/\/chat\/completions\/control$/.test(target)){controlBody=JSON.parse(opts.body);return{ok:true,status:200,statusText:'OK',headers:new Headers({'content-type':'application/json'}),text:async function(){return JSON.stringify({success:true});}};}mainBody=JSON.parse(opts.body);const body=new ReadableStream({start:function(controller){controller.enqueue(encoder.encode('data: {"id":"req-lfm","model":"lfm.gguf","choices":[{"delta":{"reasoning_content":"I should reason first"}}]}\n\n'));controller.enqueue(encoder.encode('data: {"id":"req-lfm","model":"lfm.gguf","choices":[{"delta":{"content":"final"},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":8,"total_tokens":18}}\n\ndata: [DONE]\n\n'));controller.close();}});return{ok:true,status:200,statusText:'OK',headers:new Headers({'content-type':'text/event-stream'}),body:body};};
    LF.Storage={getAiSettings:function(){return{provider:'llamacpp',endpoint:'http://127.0.0.1:8080/v1',model:'/models/LFM2.5.gguf',inactivityTimeoutMs:60000,streaming:true,thinkingMode:'off'};},getApiKey:function(){return'';}};
    LF.AIProviders={llamacpp:{id:'llamacpp',keyRequired:false,tokenParam:'max_tokens',supportsStreaming:true,supportsTemperature:true,supportsReasoningControl:true,thinkingPromptGuard:true,thinkingModes:{off:{reasoning_effort:'none',chat_template_kwargs:{enable_thinking:false,reasoning_effort:'none'}}}}};
    try{const spec=AI.buildRequest({messages:[{role:'user',content:'final only'}],stream:true,maxTokens:128,thinkingMode:'off',guardThinking:true}),result=await AI.send(spec,{label:'reasoning-control-test'});assert(mainBody.reasoning_control,true,'realtime reasoning control armed');assert(controlBody.action,'reasoning_end','reasoning end action');assert(controlBody.id,'req-lfm','request id forwarded');assert(controlBody.model,'lfm.gguf','served model forwarded');assert(result.content,'final','final content retained');assert(result.reasoningObserved,true,'ignored reasoning detected');assert(result.reasoningControlRequests,1,'one control request');assert(result.reasoningControlOk,true,'control succeeded');assert(result.httpRequests,2,'main request plus one control request');assert(calls,2,'exact HTTP request count');}
    finally{global.fetch=oldFetch;delete LF.Storage;delete LF.AIProviders;}
  };
  ...
  t['local providers never inherit a stored cloud API key'] = function () {
    LF.Storage = {
      ...
      getApiKey: function () { return 'stale-cloud-key'; }
    };
  ...
  t['buildRequest guards required provider config'] = function () {
    LF.Storage = { getAiSettings: function () { return { provider: 'openai', endpoint: '', model: '' }; }, getApiKey: function () { return ''; } };
    LF.AIProviders = { openai: { keyRequired: true } };
    let threw = false;
  ...
  t['provider rate-limit classification remains explicit without transport retry state'] = function () {
    ...
    assert(AI.limitInfo(429,'','rate limit').retryable,true,'generic HTTP rate limit remains identifiable');
  };
  ...
  t['cloud providers are not benchmarked by Detect throughput helper'] = async function () {
    LF.Storage={getAiSettings:function(){return{provider:'custom',endpoint:'https://provider.example/v1',model:'test-model'};},getApiKey:function(){return'key';}};LF.AIProviders={custom:{id:'custom',keyRequired:true}};
    try{const out=await AI.benchmarkTokensPerSecond({provider:'custom'});assert(out.supported,false,'cloud benchmark skipped');assert(out.reason,'local-providers-only','cloud skip reason');assert(out.samples,[],'no generated samples');}
    finally{delete LF.Storage;delete LF.AIProviders;}
  };
