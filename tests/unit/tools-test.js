...
module.exports=function(t,LF){
  ...
  t['assistant uses model-selected read tools before its final response']=async function(){const e=fake();state(e);let calls=0,finalMessages=null;LF.Storage={getEffectiveAction:function(id){return LF.ActionRegistry.action(id);},getAiSettings:function(){return{provider:'custom',endpoint:'http://local/v1',model:'demo',streaming:false,maxOutputTokensCap:0};},getAssistantSettings:function(){return{maxOutputTokens:2048,contextChars:12000,memoryTurns:0,memoryChars:0,messageChars:1000,memoryEnabled:false};},getEffectivePrompt:function(id){return LF.ActionRegistry.prompt(id);}};LF.AI={acceptController:function(){},estimateTokens:function(x){return Math.ceil(String(x||'').length/4);},buildRequest:function(x){return x;},send:async function(spec){calls++;if(spec.jsonMode){if(calls===1)return{content:JSON.stringify({decision:'call_tool',tool:'results.get',arguments:{},reason:'Need deterministic results'}),finishReason:'stop'};return{content:JSON.stringify({decision:'answer',tool:'',arguments:{},reason:'Enough evidence'}),finishReason:'stop'};}finalMessages=spec.messages;return{content:'DEVICE A is the current best sample at 21.2%.',finishReason:'stop',model:'demo',provider:'custom'};}};const out=await LF.ActionRunner.run('assistant.chat',{userText:'Which sample is best?'});assert(out.status,'done','status');assert(out.agentTrace.map(function(x){return x.tool;}),['results.get'],'tool trace');assert(out.result,'DEVICE A is the current best sample at 21.2%.','answer');truthy(JSON.stringify(finalMessages).indexOf('DEVICE A')>=0,'tool observation in final context');assert(calls,3,'two planner calls plus final');};

  t['assistant falls back to a normal answer when a model cannot emit the tool-choice JSON']=async function(){const e=fake();state(e);let calls=0;LF.Storage={getEffectiveAction:function(id){return LF.ActionRegistry.action(id);},getAiSettings:function(){return{provider:'nvidia',endpoint:'http://local/v1',model:'nvidia/nemotron',streaming:false,maxOutputTokensCap:0};},getAssistantSettings:function(){return{maxOutputTokens:2048,contextChars:12000,memoryTurns:0,memoryChars:0,messageChars:1000,memoryEnabled:false};},getEffectivePrompt:function(id){return LF.ActionRegistry.prompt(id);}};LF.AI={acceptController:function(){},estimateTokens:function(x){return Math.ceil(String(x||'').length/4);},buildRequest:function(x){return x;},send:async function(spec){calls++;if(spec.jsonMode)return{content:'I should inspect the results before answering.',finishReason:'stop',model:'nvidia/nemotron',provider:'nvidia'};return{content:'I can still answer from the available experiment context.',finishReason:'stop',model:'nvidia/nemotron',provider:'nvidia'};}};const out=await LF.ActionRunner.run('assistant.chat',{userText:'Summarize this experiment.'});assert(out.status,'done','status');assert(out.result,'I can still answer from the available experiment context.','fallback answer');assert(out.agentTrace,[],'no invalid tool trace');assert(out.agentPlanFallback,true,'planner fallback recorded');assert(calls,2,'invalid planner plus final answer');};
