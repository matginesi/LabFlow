...
module.exports=function(t,LF){
  ...
  t['assistant Knowledge Base tool is read-only RAG with explicit external provenance']=function(){
    const e=fake();state(e),previous=LF.KnowledgeBase;LF.KnowledgeBase={status:function(){return{connected:true,records:34};},search:function(query){return[{id:'kb_sno2',kind:'material',name:'SnO2',data:{role:'ETL'},sources:[{doi:'10.1000/source'}],retrieval:{score:12,matched_terms:['sno2']}}];}};
    try{const out=LF.ToolRegistry.execute('knowledge.search',{query:'SnO2 electron contact',limit:4},{exp:e,agent:true});assert(out.connected,true,'connected');assert(out.total_library_records,34,'library size');assert(out.records[0].id,'kb_sno2','retrieved record');assert(out.records[0].data.role,'ETL','record data retained');const listed=LF.ToolRegistry.describeForAgent(['knowledge.search']);assert(listed[0].id,'knowledge.search','agent-visible tool');}
