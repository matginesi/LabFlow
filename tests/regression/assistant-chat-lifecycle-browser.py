...
with sync_playwright() as p:
    ...
    check(page.locator('.chat-thinking-dot').count()==1, 'first request must show one spinner')
    ...
    check(page.locator('.chat-thinking-dot').count()==0, 'first content must remove spinner')
    ...
    check(page.locator('.chat-thinking-dot').count()==1, 'second request must show exactly one spinner')
    ...
    check(page.locator('.chat-thinking-dot').count()==0, 'second first content must remove spinner')
    ...
    check(page.locator('.chat-thinking-dot').count()==0, 'second completion must leave no spinner')
    ...
    page.wait_for_function("!LabFlow.Assistant.isActive()")
    check(page.locator('.chat-thinking-dot').count()==0, 'abort must leave no spinner')
