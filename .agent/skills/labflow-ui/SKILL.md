...
Cabinet is a browser-local library of **reusable scientific recipes and definitions**, not inventory/LIMS and not a second experiment database. Its researcher-facing mental model is explicit: **Save → Reuse → the experiment keeps its own copy**. When an experiment is open, make it easy to capture a formulation, device stack or process recipe directly from Design; otherwise explain the purpose before showing library-management controls. Incomplete resources can be edited but must not be presented as applicable matches. Copying Cabinet content into Design uses detached snapshots and must preserve chemistry/stack/process content, so later Cabinet edits never rewrite past experiments. Cabinet belongs to the scientific canvas, not the dark navigation chrome: use shared LabFlow fields, buttons, badges, notices and selects; resource kinds may use restrained chart-token accents only for recognition. Keep one purpose/capture area, one search/type/create toolbar, one locally scrollable resource shelf and one detail editor, with portable backup behind progressive disclosure. The canonical layout is **purpose/capture → responsive shelf → full-width editor**. The shelf uses compact selectable resource tiles (not a KPI/card dashboard and not a table): 3 columns on wide canvas, 2 on tablet, 1 on phone. At narrow widths the library toolbar stacks, scientific fields become one column, editor actions become full-width controls, and page-level horizontal scrolling remains zero. Do not add inventory quantities, stock tracking, a second toolbar, a permanent master-detail split, or compatibility CSS for retired Cabinet layouts.
...
The Knowledge Base management surface uses one search/filter toolbar, one bounded reference catalogue and one detail editor built from standard panel/field/button/badge/notice primitives. The user model is **Built-in library + My JSONL**: bundled references are read-only; editable user references are stored as plain JSONL (one object per line) and can be opened/saved directly where the browser permits, with a normal download/file-input fallback elsewhere. Do not add a database merely for custom knowledge. JSONL import is fail-closed: bound file size and entry count, report malformed line numbers, reject duplicate explicit IDs inside the file, and never silently overwrite bundled IDs. Draft entries may be incomplete but must stay out of AI retrieval; a Ready/active reference requires traceable sourcing. Keep aliases, related IDs and other implementation-oriented metadata under progressive disclosure. Do not turn the KB into a dashboard, graph browser, RAG console, inventory UI or separate design system. Source citations use normal readable UI sizes and progressive disclosure where needed.
