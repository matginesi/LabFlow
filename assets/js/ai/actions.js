...
function retryable(step,err,retryNo,truncationRetryNo){
  ...
  const configured=step.max_retries==null?AUTO_RETRY_DELAYS.length:Math.max(0,Math.min(AUTO_RETRY_DELAYS.length,Number(step.max_retries)||0));return Math.max(0,Number(retryNo)||0)<configured;
}
...
function fitWorkItemToOutput(step,workItem,hardCap,capability,settings){
  const requestedWords=positive(workItem&&workItem.target_words);if(!requestedWords)return workItem;
  const profile=tokenProfile(step,workItem,hardCap),ceiling=outputCeiling(capability,settings,hardCap);if(!ceiling||ceiling>=profile.requestTokens)return workItem;
  const safeTargetTokens=Math.max(64,Math.floor((ceiling-256)/1.35)),supportedWords=Math.max(80,Math.floor((safeTargetTokens-220)/1.65));if(supportedWords>=requestedWords)return workItem;
  const originalMin=positive(workItem.min_words)||Math.floor(requestedWords*.72),originalMax=positive(workItem.max_words)||Math.ceil(requestedWords*1.28),target=Math.max(80,supportedWords),min=Math.max(60,Math.min(originalMin,Math.floor(target*.72))),max=Math.max(target,Math.min(originalMax,Math.ceil(target*1.18)));
  return Object.assign({},workItem,{requested_target_words:requestedWords,target_words:target,min_words:min,max_words:max,output_budget_adjusted:true});
}
...
function providerCompletionCeiling(capability,settings,inputTokens){
  const values=[],model=positive(capability&&capability.maxOutputTokens),context=positive(capability&&capability.contextWindow),forced=positive(settings&&settings.maxOutputTokensCap),input=Math.max(0,Number(inputTokens)||0);
  if(model)values.push(model);if(context)values.push(Math.max(16,context-input-512));if(forced)values.push(forced);return values.length?Math.min.apply(Math,values):null;
}
...
async function runAi(run,step,opts,workItem,workIndex,workTotal){
  const key=step.id+(workTotal>1?':'+workIndex:''),settings=LF.Storage.getAiSettings(),assistantCap=run.actionId==='assistant.chat'&&LF.Storage.getAssistantSettings?positive(LF.Storage.getAssistantSettings().maxOutputTokens):null,declaredCap=positive(step.max_output_tokens)||positive(run.def.max_output_tokens)||4096,actionCap=assistantCap?Math.min(assistantCap,declaredCap):declaredCap;
...
async function runUnitWithRetry(run,step,opts,item,index,total){
  ...
  while(true){
    ...
    try{return await runOne(run,step,opts,activeItem,index,total);}
    catch(err){
      const code=classify(err),isTruncation=code==='MODEL_OUTPUT_TRUNCATED',can=retryable(step,err,semanticRetryNo,truncationRetryNo),kind=isTruncation?'truncation-recovery':'semantic-retry';
      run.attempts.push({step:step.id,index:run.currentIndex,workIndex:index,attempt:totalAttemptNo+1,status:can?'retry':'error',retryKind:can?kind:'',code:code,message:String(err&&err.message||err)});
      if(!can)throw err;
      if(isTruncation){
        activeItem=shrinkTruncatedWorkItem(activeItem,err&&err.providerResponse);
        run.retryCompletionBudget[key]=truncationRetryBudget(err,tokenProfile(step,activeItem,positive(step.max_output_tokens)||null));
        truncationRetryNo++;
