(function(){
  ...
  function automaticCleanupState(exp,extra){
    ...
    exp.autoCleanup={applied:automaticPatches.length,pending:fixes.length,pendingTargets:pendingTargets,lastApplied:Number(extra&&extra.lastApplied||0),withdrawn:Number(extra&&extra.withdrawn||0),targets:Number(extra&&extra.targets!=null?extra.targets:pendingTargets),items:fixes.map(function(x){return{patch_type:x.patch_type,target:x.target,after:x.after,reason:x.reason,evidence:(x.evidence||[]).slice(0,3)};}),appliedItems:automaticPatches.map(function(x){return{patch_type:x.patchType,target:x.target&&x.target.id||x.target,after:x.to,reason:x.reason,evidence:(x.evidence||[]).slice(0,3)};}),updatedAt:new Date().toISOString()};return exp.autoCleanup;
  }
  function prepareAutomaticSafeFixes(exp){const withdrawn=withdrawAutomaticExclusions(exp);return automaticCleanupState(exp,{withdrawn:withdrawn});}
