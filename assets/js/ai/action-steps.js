(function(){
  ...
  function designDecisionMetric(proposal){
    ...
    const value=candidates.length?Math.round(candidates.reduce(function(sum,item){return sum+item.score;},0)/candidates.length*100):0,experimentBacked=candidates.filter(function(item){return item.source==='experiment';}).length,modelInferred=candidates.filter(function(item){return item.source==='model_inference';}).length;
    return{value:value,label:value>=80?'high':value>=60?'moderate':'low',decisions:candidates.length,experimentBacked:experimentBacked,evidenceBacked:experimentBacked,modelInferred:modelInferred,autoApply:candidates.filter(function(item){return item.autoApply;}).length,review:candidates.filter(function(item){return !item.autoApply;}).length,method:'proposed_field_confidence_mean',estimated:true};
  }
  ...
  function canonicalDesignSource(item,inherited){let raw=String(item&&item.provenance_kind||item&&item.provenanceKind||item&&item.source||inherited||'').toLowerCase().trim();if(raw==='raw_evidence'||raw==='source'||raw==='evidence'||raw==='mixed')raw=String(item&&item.evidence||'').trim()?'experiment':'model_inference';return raw==='experiment'?'experiment':'model_inference';}
  ...
  function applicableDesignFields(proposal,unknownFields){
    ...
    if(wanted.has('solutions')&&solutions.some(function(sol){return String(sol&&sol.name||sol&&sol.solutes||sol&&sol.solvents||'').trim();}))fields.push('solutions');
  ...
  function resultsSelection(){const bp=LF.State&&LF.State.state&&LF.State.state.boxPlot||{};return{groups:(bp.groups||[]).map(String).filter(Boolean),metric:String(bp.metric||'eff'),direction:String(bp.direction||'both'),eligibleOnly:bp.eligibleOnly!==false};}
  ...
  const steps={
    ...
    'results.validate-comparison':function(ctx){
      const value=ctx.candidate||ctx.outputs.compare||ctx.lastResult||{},selection=resultsSelection(ctx.exp),expected=selection.groups||[];
    ...
    'design.validate-coverage':function(ctx){
      ...
      if(proposal.devices[0]){
        proposal.devices[0].sample_names=wanted.slice();
        if((proposal.solutions||[]).length&&!(proposal.devices[0].solution_names||[]).length)proposal.devices[0].solution_names=(proposal.solutions||[]).map(function(sol){return sol.name;}).filter(Boolean);
      }
  ...
  function applyDesignSolution(exp,src,manual){const fields=['name','role','solutes','solvents','concentration','additives','preparation'],name=String(src.name||'').toLowerCase().trim(),key=(String(src.name||'')+'|'+String(src.role||'')).toLowerCase().trim();let dst=(exp.design.solutions||[]).find(function(sol){const solName=String(sol.name||'').toLowerCase().trim(),solKey=(String(sol.name||'')+'|'+String(sol.role||'')).toLowerCase().trim();return(name&&solName===name)||solKey===key;}),created=false;if(!dst){dst=LF.DomainSchema.create('design_solution',{status:'ai_inferred',provenanceKind:'model_inference'});exp.design.solutions.push(dst);created=true;}let n=fillMissing(dst,src,fields,src,null,manual);if(n)n+=fillMetadata(dst,src);if(!n&&created)exp.design.solutions=exp.design.solutions.filter(function(x){return x!==dst;});if(n){dst.status=dst.status==='user_confirmed'?'user_confirmed':'ai_inferred';dst.aiAssisted=true;dst.aiAssistedAt=new Date().toISOString();src.applied=decisionsComplete(src,fields);src.appliedSome=true;src.decision=src.applied?'accepted':'pending';}return n;}
  ...
  function acceptDesignProposal(exp,deviceId){const id=String(deviceId||''),p=LF.ActionData&&LF.ActionData.proposal(exp,'design.infer',id);if(!p)throw new Error('No AI suggestion is available for this experiment.');let changed=0;(p.solutions||[]).forEach(function(sol){changed+=applyDesignSolution(exp,sol,true);});const d=p.devices&&p.devices[0];if(d){changed+=applyDesignDevice(exp,d,'solutions',true,p);changed+=applyDesignDevice(exp,d,'stack',true,p);}const target=(exp.design&&exp.design.devices||[]).find(function(x){return String(x.id)===id;});if(target){target.validatedAt=new Date().toISOString();target.validationSource='researcher_accept_ai';target.status='user_confirmed';(target.stack||[]).forEach(function(layer){if(layer.status==='ai_inferred')layer.status='user_confirmed';});(target.solutionIds||[]).forEach(function(solutionId){const sol=(exp.design.solutions||[]).find(function(x){return String(x.id)===String(solutionId);});if(sol&&sol.status==='ai_inferred')sol.status='user_confirmed';});}LF.ActionData.setStatus(exp,'design.infer',id,{state:'accepted',updatedAt:new Date().toISOString(),message:''});LF.ActionData.removeProposal(exp,'design.infer',id);exp.design.status='reviewing';return{changed:changed,deviceId:id};}
  function acceptAllDesignProposals(exp){const map=LF.ActionData?LF.ActionData.proposals(exp,'design.infer'):{},ids=Object.keys(map||{});let changed=0,accepted=0,failed=[];ids.forEach(function(id){try{const out=acceptDesignProposal(exp,id);changed+=out.changed;accepted++;}catch(err){failed.push({id:id,message:err&&err.message||String(err)});}});return{changed:changed,accepted:accepted,failed:failed};}
