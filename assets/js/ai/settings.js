(function () {
  ...
  */
  async function testConnection(button) {
    ...
    try {
      LF.UI.activityUpdate({stage:'Waiting for provider', indeterminate:true, message:settings.provider==='zai'?'The request is going through the local LabFlow Z.AI relay.':'The provider request is in progress.'});
      ...
      LF.UI.activityError(error, {
        response: 'What happened\n' + (error.message || String(error)) + (error.providerResponse ? '\n\nProvider response\n' + error.providerResponse : '') + '\n\nWhat to do next\n' + summary.next + (settings.provider==='zai'?'\n\nZ.AI endpoint\nLabFlow uses the official General API endpoint https://api.z.ai/api/paas/v4/chat/completions through its bundled same-origin relay. Start LabFlow with tools/serve_static.py so the relay is available.':'') ,
