  ...
  async function loadModels(options) {
    ...
    try{
      ...
      if(localBenchmark&&LF.AI.benchmarkTokensPerSecond){
        ...
        try{benchmark=await LF.AI.benchmarkTokensPerSecond({provider:providerId,endpoint:endpoint,model:selected,apiKey:apiKey,samples:3,maxTokens:64,warmupTokens:24});const sampleText=benchmark.samples.map(function(item){return Number(item.tokensPerSecond).toFixed(1);}).join(' / '),estimateSuffix=benchmark.estimated?' (token count estimated)':'';details['Average throughput']=Number(benchmark.averageTokensPerSecond).toFixed(2)+' tok/s'+estimateSuffix;details['Throughput range']=Number(benchmark.minTokensPerSecond).toFixed(2)+'–'+Number(benchmark.maxTokensPerSecond).toFixed(2)+' tok/s';details['Throughput samples']=sampleText+' tok/s';activity.activityUpdate({stepId:'benchmark',stepStatus:'done',stepNote:Number(benchmark.averageTokensPerSecond).toFixed(2)+' tok/s avg'+(benchmark.estimated?' · estimated tokens':'')+' · '+sampleText,stage:'Local throughput measured',progress:.91,details:details});}
        catch(error){benchmarkError=error;details['Average throughput']='unavailable';activity.activityUpdate({stepId:'benchmark',stepStatus:'done',stepNote:'unavailable',stage:'Throughput benchmark unavailable',progress:.91,message:'Capability detection succeeded, but the optional local throughput benchmark failed: '+(error.message||String(error)),details:details});Log.warn('benchmark.failed',{provider:providerId,model:selected,error:error});}
      }
      ...
      if(hint)hint.textContent=(models.length?models.length+' model'+(models.length===1?'':'s')+' available · ':'')+capText+' · '+reasoningText+benchmarkText+(runtimeProfileText?' · '+runtimeProfileText:'')+(cap&&cap.source?' · '+cap.source:'')+(listError?' · model list unavailable; manual model entry enabled':'')+(benchmarkError?' · throughput unavailable':'');
      activity.activityUpdate({stepId:'apply',stepStatus:'done',stage:'Provider metadata ready',progress:.97,details:details});
      Log.info('models.loaded',{provider:providerId,count:models.length,model:selected,capability:cap,throughput:benchmark});
      activity.activityFinish({message:cap&&cap.runtimeProfileStatus==='mismatch'?'Provider metadata detected · llama.cpp runtime differs from the LabFlow profile.':'Provider metadata detection completed.',response:(models.length?models.length+' models detected. ':'')+capText+'.'+(benchmark?' Local generation throughput: '+Number(benchmark.averageTokensPerSecond).toFixed(2)+' tok/s average'+(benchmark.estimated?' using estimated token counts':'')+' ('+benchmark.samples.map(function(item){return Number(item.tokensPerSecond).toFixed(1);}).join(', ')+' tok/s).':'')+(runtimeProfileText?' '+runtimeProfileText+'.':'')+' LabFlow Action requests remain much smaller and use explicit per-Action input/output caps.'+(cap&&cap.runtimeProfileStatus==='mismatch'?'\n\nRecommended llama.cpp runtime for this LabFlow profile: --parallel 1 --ctx-size 65536. Detect never divides the n_ctx reported by /props a second time.':'')+(listError?'\n\nThe model catalogue itself was unavailable, but the configured model was still probed.':'')+(benchmarkError?'\n\nThe optional local throughput benchmark was unavailable: '+(benchmarkError.message||String(benchmarkError)):''),details:details,holdMs:0});
