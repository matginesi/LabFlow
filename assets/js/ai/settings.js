(function () {
  ...
  async function detectModel(options) {
    ...
    if(useActivity)startProviderActivity('Check AI connection','Connection and model check',draft,[
    ...
    try{
      ...
      if(useActivity)failProviderActivity(error,{stage:'Validation failed',message:'Connection check could not start.',response:error.message||String(error),details:{Phase:'validate',Diagnostic:diagnosticId}});
      return[];
    }
  ...
  async function testConnection(button) {
    ...
    startProviderActivity('Save & test AI connection','Live connection check before saving',draft,[
      {id:'validate',label:'Validate visible settings',status:'active'},
      {id:'probe',label:'Run live LLM probe',status:'pending'},
