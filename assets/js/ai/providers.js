(function () {
  ...
  LF.AIProviders = {
    ...
    ollama: {
      id:'ollama', name:'Ollama', local:true, endpoint:'http://127.0.0.1:11434/v1', model:'gemma3', keyRequired:false, modelSelect:true, modelSelectLabel:'Ollama model', supportsStreaming:true, tokenParam:'max_tokens', supportsTemperature:true, supportsJsonMode:true, thinkingPromptGuard:true, thinkingModes:{off:{reasoning_effort:'none'},on:{reasoning_effort:'medium'}}, requestTimeoutMs:300000, connectionTestTimeoutMs:15000,
      note:'OpenAI-compatible Ollama endpoint. Detect reads installed/running models. For another device on the same network, expose Ollama on a LAN address and allow the LabFlow page origin.'
    },
    lmstudio: {
      ...
      note:'OpenAI-compatible LM Studio endpoint. Detect reads the native model API first and falls back to /v1/models. For another device on the same network, enable Serve on Local Network and CORS.'
    },
    llamacpp: {
      ...
      note:'OpenAI-compatible llama-server endpoint. Detect reads /v1/models and /props, including the effective context/slot configuration. For another device on the same network, llama-server must listen on a LAN interface and allow the LabFlow page origin.'
    },
    custom: {
