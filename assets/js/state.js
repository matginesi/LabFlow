...
if(!entry||!Array.isArray(entry.runs))return;
      ...
      const compact=Object.assign({},run);delete compact.outputs;delete compact.result;
      if(compact.requestMeta&&typeof compact.requestMeta==='object'){const meta={};
      Object.keys(compact.requestMeta).slice(-24).forEach(function(key){const m=compact.requestMeta[key]||{};
      meta[key]={model:m.model||'',provider:m.provider||'',thinkingMode:m.thinkingMode||'auto',requestId:m.requestId||'',
      requestLogId:m.requestLogId||'',latencyMs:m.latencyMs,ttftMs:m.ttftMs,tokensPerSecond:m.tokensPerSecond,
      streamed:!!m.streamed,usage:m.usage||null,finishReason:m.finishReason||''};});compact.requestMeta=meta;
      }if(Array.isArray(compact.attempts))compact.attempts=compact.attempts.slice(-8).map(function(x){return Object.assign({}
      ,x,{message:String(x&&x.message||'').slice(0,900)});});return compact;});});
