...
registerRecord('file',{
  ...
  normalize:function(r){r=obj(r);if(!r.id)r.id=uid('f');r.kind='file';r.path=text(r.path||r.rawPath||r.name);
r.rawPath=text(r.rawPath||r.path);r.name=text(r.name||r.canonicalName||r.rawName||
    r.path.split('/').filter(Boolean).pop());r.rawName=text(r.rawName||r.name);
    r.canonicalName=text(r.canonicalName||r.name);r.canonicalPath=text(r.canonicalPath||r.path);
    r.extension=text(r.extension||((r.name.match(/\.[^.]+$/)||[''])[0])).toLowerCase();
    r.family=r.family==null?null:text(r.family);r.type=text(r.type||'unknown');r.size=Number(r.size)||0;
    r.sha256=text(r.sha256);r.unreadable=!!r.unreadable;return r;}
});
...
registerRecord('format_evidence',{
  ...
  normalize:function(r){r=obj(r);if(!r.id)r.id=uid('fmt');r.kind='format_evidence';r.path=text(r.path);
r.detectedFamily=text(r.detectedFamily||'unknown');r.extension=text(r.extension);r.characters=Number(r.characters)||0;
    r.lines=Number(r.lines)||0;r.delimiterCounts=mergeDefaults({tabs:0,semicolons:0,commas:0},r.delimiterCounts);
    r.replacementCharacters=Number(r.replacementCharacters)||0;r.rawLines=arr(r.rawLines).map(text);return r;}
});
registerRecord('auxiliary_evidence',{
  ...
  normalize:function(r){r=obj(r);if(!r.id)r.id=uid('aux');r.kind='auxiliary_evidence';r.type=text(r.type||'unknown');
r.file=text(r.file);r.path=text(r.path);r.sample=text(r.sample);r.group=text(r.group);
    r.experiment=text(r.experiment||r.group);r.isRef=!!r.isRef;r.meta=obj(r.meta);
    r.dataColumns=arr(r.dataColumns).map(text);r.rowCount=Number(r.rowCount)||0;r.experimentId=text(r.experimentId);
    r.sampleId=text(r.sampleId);r.runId=text(r.runId);r.position=text(r.position);r.cell=text(r.cell);return r;}
});
...
registerRecord('sample',{
  ...
  normalize:function(r){r=obj(r);if(!r.id)r.id=uid('sample');r.kind='sample';r.name=text(r.name);
r.rawName=text(r.rawName||r.name);r.aliases=Array.from(new Set(arr(r.aliases).concat([r.rawName,
    r.name]).map(text).filter(Boolean)));r.experimentId=text(r.experimentId);r.experiment=text(r.experiment||r.group);
    r.group=text(r.group||r.experiment);r.position=text(r.position);r.cell=text(r.cell);r.isRef=!!r.isRef;
    r.runIds=arr(r.runIds).map(text);r.measurementIds=arr(r.measurementIds).map(text);r.meta=obj(r.meta);return r;}
});
registerRecord('run',{
...
registerRecord('finding',{
  ...
  normalize:function(r){r=obj(r);if(!r.id)r.id=uid('finding');r.kind='finding';r.severity=text(r.severity||'info');
r.type=text(r.type||'unknown');r.title=text(r.title);r.detail=text(r.detail);r.target=text(r.target);
    r.evidence=arr(r.evidence);r.status=text(r.status||'open');r.source=text(r.source||'deterministic');
    r.measurementId=text(r.measurementId);r.resolvedAt=r.resolvedAt||null;r.resolvedBy=text(r.resolvedBy);return r;}
});
registerRecord('block',{
  ...
  normalize:function(r){r=obj(r);if(!r.id)r.id=uid('b');r.kind='block';r.type=text(r.type||'table');
r.family=r.family==null?null:text(r.family);r.name=text(r.name);r.direction=r.direction==null?null:text(r.direction);
    r.file=mergeDefaults({id:'',path:'',locator:null},r.file);r.refs=arr(r.refs).map(function(ref){ref=obj(ref);
    return{kind:text(ref.kind),id:text(ref.id)};}).filter(function(ref){return ref.kind&&ref.id;});r.schema=obj(r.schema);
    r.schema.columns=arr(r.schema.columns);r.data=obj(r.data);r.data.header=arr(r.data.header);r.data.rows=arr(r.data.rows);
    r.metadata=obj(r.metadata);return r;}
});
registerRecord('patch',{
  ...
  normalize:function(r){r=obj(r);if(!r.id)r.id=uid('patch');r.kind='patch';
r.patchType=text(r.patchType||r.type||'value_change');const target=obj(r.target);
    if(typeof r.target==='string')r.target={kind:text(r.targetKind),id:text(r.target)};
    else r.target={kind:text(target.kind||r.targetKind),id:text(target.id||r.targetId)};
    if(r.blockId&&!r.target.id)r.target={kind:'block',id:text(r.blockId)};r.operation=text(r.operation||'set');
    r.field=text(r.field);r.source=text(r.source||'system');r.reason=text(r.reason);r.evidence=arr(r.evidence);
    r.findingId=text(r.findingId);r.confidence=finiteOrNull(r.confidence);r.status=text(r.status||'applied');
    r.reviewStatus=text(r.reviewStatus||'accepted');r.reviewedBy=text(r.reviewedBy);r.createdAt=r.createdAt||now();
    r.appliedAt=r.appliedAt||null;delete r.type;delete r.blockId;delete r.targetKind;delete r.targetId;return r;}
});
...
function normalizeRoot(exp){
  ...
  exp.design=mergeDefaults({status:'unknown',solutions:[],process:{coating:'',annealing:'',atmosphere:'',notes:''}
,stack:[],devices:[],evidenceSummary:{sourceRecords:0,parsedRecords:0,samplesCovered:0,experimentsRecovered:0,
    sourceAvailable:false}},exp.design);exp.design.solutions=arr(exp.design.solutions).map(function(x){
    return normalize('design_solution',x);});
    exp.design.stack=arr(exp.design.stack).map(function(x){return normalize('design_layer',x);});
    exp.design.devices=arr(exp.design.devices).map(function(x){return normalize('design_device',x);});
    exp.design.process=mergeDefaults({coating:'',annealing:'',atmosphere:'',notes:''},exp.design.process);
    exp.design.evidenceSummary=mergeDefaults({
    sourceRecords:0,parsedRecords:0,samplesCovered:0,experimentsRecovered:0,sourceAvailable:false}
    ,exp.design.evidenceSummary);
...
function cloneValue(value){if(value==null||typeof value!=='object')return value;
if(value instanceof ArrayBuffer)return value.slice(0);
  if(typeof ArrayBuffer!=='undefined'&&ArrayBuffer.isView&&ArrayBuffer.isView(value)){
  const copy=value.buffer.slice(value.byteOffset,value.byteOffset+value.byteLength);return new value.constructor(copy);
