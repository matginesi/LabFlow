...
function removeProposal(exp,actionId,targetId){const root=ensure(exp),id=key(actionId);
if(targetId==null||targetId===''){const had=Object.prototype.hasOwnProperty.call(root.proposals,id);
  delete root.proposals[id];return had;}const map=root.proposals[id];if(!map||typeof map!=='object')return false;
  const target=String(targetId),had=Object.prototype.hasOwnProperty.call(map,target);delete map[target];
  if(!Object.keys(map).length)delete root.proposals[id];return had;}
...
function removeStatus(exp,actionId,targetId){const root=ensure(exp),id=key(actionId);
if(targetId==null||targetId===''){const had=Object.prototype.hasOwnProperty.call(root.status,id);delete root.status[id];
  return had;}const map=root.status[id];if(!map||typeof map!=='object')return false;
  const target=String(targetId),had=Object.prototype.hasOwnProperty.call(map,target);delete map[target];
  if(!Object.keys(map).length)delete root.status[id];return had;}
