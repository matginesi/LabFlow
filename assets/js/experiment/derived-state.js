...
const REGISTRY=[];
function removePath(root,path){const parts=String(path||'').split('.').filter(Boolean);if(!parts.length)return false;let node=root;for(let i=0;i<parts.length-1;i++){if(!node||typeof node!=='object')return false;node=node[parts[i]];}if(!node||typeof node!=='object'||!Object.prototype.hasOwnProperty.call(node,parts[parts.length-1]))return false;delete node[parts[parts.length-1]];return true;}
function register(id,spec){if(!id||!spec)throw new Error('DerivedState.register requires id and specification.');
if(REGISTRY.some(function(x){return x.id===id;}))throw new Error('Duplicate derived projection: '+id);
  const row=Object.assign({id:id,dependsOn:[],paths:[],description:'',invalidate:null},spec);
...
description:'Design projections/proposals tied to current experiment/design state.',invalidate:function(exp){
  ...
  LF.ActionData.clear(exp,'design.infer');return had;}});
register('nomad-projection',{dependsOn:['dataset','analysis','validation','design','metadata'],
description:'NOMAD mapping/validation state derived from the LabFlow Data.',invalidate:function(exp,scope){
  exp.nomad=exp.nomad||{};const had=!!(exp.nomad.validation||exp.nomad.mappingPlan||exp.nomad.staleSince||
  exp.nomad.upload);exp.nomad.validation=null;delete exp.nomad.mappingPlan;exp.nomad.staleSince=new Date().toISOString();
  exp.nomad.staleReason=scope;if(exp.nomad.upload){exp.nomad.upload.stale=true;exp.nomad.upload.staleReason=scope;
  }return had||true;}});
LF.DerivedState={register:register,invalidate:invalidate,describe:describe};
}());
