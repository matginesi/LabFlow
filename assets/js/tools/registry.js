...
function expOf(ctx){return ctx&&ctx.exp||(LF.State&&LF.State.state&&LF.State.state.experiment)||null;}
...
function register(id,def){
  ...
  if(defs[id])throw new Error('Tool already registered: '+id);
  if(!def||typeof def!=='object'||typeof def.handler!=='function')throw new Error('Tool '+id+' requires a handler.');
  defs[id]=Object.assign({title:id,domain:'internal',access:'read',agent_visible:false,description:'LabFlow tool.',input:{type:'object',properties:{}}},def);
  return definition(id);
}
function registerActionStep(id,meta){
  meta=meta||{};
  return register(id,{
    title:meta.title||id,
    domain:meta.domain||'internal',
    access:meta.access||'read',
    agent_visible:false,
    description:meta.description||'Internal deterministic LabFlow service used by Actions.',
    input:{type:'object',properties:{}},
    handler:function(args,ctx){const fn=LF.ActionSteps&&LF.ActionSteps[id];if(!fn)throw new Error('Tool implementation is not registered: '+id);return fn(ctx||{});}
  });
}
if(LF.ActionStepTools&&typeof LF.ActionStepTools==='object'){
  Object.keys(LF.ActionStepTools).forEach(function(id){if(!defs[id])registerActionStep(id,LF.ActionStepTools[id]);});
}
