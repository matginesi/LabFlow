(function () {
  ...
  function categoryOf(entry) {
    ...
    if ((entry.scope === 'ai' && /^(?:request|response|models|metadata|connection-test|network|capability)\./.test(entry.event)) || (entry.scope === 'ai-settings' && /^(?:detect|connection-test)\./.test(entry.event))) return 'api';
    if (/^ui\.|^route\.|navigation/.test(entry.scope + '.' + entry.event)) return 'ui';
    if (/parser|analysis|experiment|design|nomad|dataset/.test(entry.scope + '.' + entry.event)) return 'data';
    return 'runtime';
  }
  ...
  function transactionDetail(tx) {
    ...
    const providerId=String(start.provider||failed.provider||rejected.provider||parsed.provider||''),rawModel=start.model||failed.model||rejected.model||parsed.model||'',displayModel=C.modelDisplayName?C.modelDisplayName(providerId,rawModel):rawModel,transport=end.transport||failed.transport||start.transport||'',phase=failed.phase||end.phase||start.phase||'chat';
    return '<dl class="log-facts">'+fact('Correlation ID',tx.id)+fact('Provider',providerId)+fact('Phase',phase)+fact('Transport',transport)+fact('Provider request ID',failed.requestId||rejected.requestId||response.requestId||parsed.requestId)+fact('Model',displayModel)+fact('Status',state+(status?' · HTTP '+status:''))+fact('Duration',duration?duration+' ms':'')+fact('Finish reason',rejected.finishReason||parsed.finishReason)+fact('Category',rejected.category)+fact('Tokens',usage?C.safeJson(usage,0):'')+'</dl><div class="log-payload-grid">'+codeBlock('Headers sent',start.request&&start.request.headers||failed.request&&failed.request.headers)+codeBlock('Request body / messages',start.request&&start.request.body||failed.request&&failed.request.body)+codeBlock('Response headers',response.headers)+codeBlock('Raw provider response',rawResponse,tx.failed||tx.rejected?'danger':'')+codeBlock('Failure detail',failed.error||failed.providerMessage||rejected.error,'danger')+codeBlock('Parsed metadata',parsed)+'</div>';
  }
  ...
  function providerChecks(entries) {
    const map={};
    entries.forEach(function(entry){
      if(entry.scope!=='ai-settings'||!/^(?:detect|connection-test)\./.test(entry.event))return;
      const data=entry.data||{},id=String(data.diagnosticId||'');if(!id)return;
      const row=map[id]||(map[id]={id:id,start:null,end:null,stages:[]});
      if(/\.start$/.test(entry.event))row.start=entry;
      if(entry.event==='detect.stage')row.stages.push(entry);
      if(/\.(?:ok|failed|rate-limited)$/.test(entry.event))row.end=entry;
    });
    return Object.keys(map).map(function(id){return map[id];}).sort(function(a,b){const at=(a.end||a.start||{}).ts||'',bt=(b.end||b.start||{}).ts||'';return String(bt).localeCompare(String(at));});
  }
  function providerCheckCard(check) {
    const start=check.start&&check.start.data||{},end=check.end&&check.end.data||{},entry=check.end||check.start||{},failed=check.end&&/failed$/.test(check.end.event),limited=check.end&&/rate-limited$/.test(check.end.event),ok=check.end&&/ok$/.test(check.end.event),tone=failed?'danger':limited?'warning':ok?'success':'info';
    const provider=end.provider||start.provider||'',endpoint=end.endpoint||start.endpoint||'',model=end.model||start.model||'',phase=end.phase||(check.stages.length&&check.stages[check.stages.length-1].data&&check.stages[check.stages.length-1].data.phase)||'',transport=end.transport||start.transport||'',message=end.error&&end.error.message||end.message||'';
    return '<div class="provider-check"><div class="provider-check-head"><span class="badge '+tone+'">'+C.escapeHtml(failed?'FAILED':limited?'LIMITED':ok?'OK':'RUNNING')+'</span><strong>'+C.escapeHtml(provider||'provider')+'</strong><span class="mono">'+C.escapeHtml(check.id)+'</span><time>'+C.escapeHtml(entry.ts?new Date(entry.ts).toLocaleTimeString():'')+'</time></div><div class="provider-check-main"><span>'+C.escapeHtml(endpoint||'—')+'</span><span>'+C.escapeHtml(model?(C.modelDisplayName?C.modelDisplayName(provider,model):model):'—')+'</span></div><div class="meta">'+C.escapeHtml((phase?'phase: '+phase:'')+(transport?' · '+transport:'')+(message?' · '+message:''))+'</div></div>';
  }
  ...
  function render() {
    ...
    const environment=LF.Logger.environmentSnapshot(),checks=providerChecks(LF.Logger.entries());
    const relay=LF.AI&&LF.AI.relayStatus?LF.AI.relayStatus():null;
    ...
    return '<section class="page logs-page">'+
      ...
      '<section class="panel logs-health"><div class="panel-head"><div><h2 class="h2">Runtime snapshot</h2><div class="meta mono">'+C.escapeHtml(environment.sessionId)+'</div></div><div class="spacer"></div><span class="badge '+(environment.browser.online?'success':'danger')+'">'+(environment.browser.online?'Online':'Offline')+'</span></div><dl class="log-facts">'+fact('Build',environment.app.build||'dev')+fact('Started',environment.startedAt)+fact('Route',environment.app.route)+fact('Protocol',environment.page.protocol)+fact('Viewport',environment.viewport.width+' × '+environment.viewport.height)+fact('Browser language',environment.browser.language)+fact('Relay',relay&&relay.checkedAt?(relay.available?'available':'not available'):'not checked')+fact('Buffer level',environment.logging.level.toUpperCase())+'</dl></section>'+
      '<section class="panel"><div class="panel-head"><div><h2 class="h2">Provider checks</h2><div class="meta">Detect and Save & test, grouped by diagnostic ID and final phase</div></div><div class="spacer"></div><span class="badge info">'+checks.length+' checks</span></div><div class="provider-check-list">'+(checks.length?checks.slice(0,8).map(providerCheckCard).join(''):'<div class="empty compact-empty">No provider checks in this session.</div>')+'</div></section>'+
