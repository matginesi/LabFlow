(function () {
  ...
  function routeTitle(route) {
    return ({'experiment-import':'Upload','experiment-understand':'Review data','experiment-results':'Results','experiment-design':'Design','experiment-report':'Report','experiment-nomad':'NOMAD',settings:'Settings',logs:'Logs','ui-kit':'UI Kit'})[route] || 'LabFlow';
  }
  ...
  function needExperiment() {
    return '<section class="page start-page">'+workflowHead('Upload experiment','Load the original laboratory ZIP to begin the RAW → Review → Results → Design → Report → NOMAD workflow.')+
      ...
      '<div class="notice info upload-notice"><strong>Non-destructive workflow.</strong> RAW bytes are snapshotted at import and never renamed, rewritten or autosaved. Corrections affect only the Working Copy; Save/Export creates a new file explicitly.</div></section>';
  }
