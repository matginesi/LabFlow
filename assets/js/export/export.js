(function () {
  ...
  async function saveZip(exp){if(!window.JSZip)throw new Error('JSZip is unavailable.');const zip=new JSZip(),data=snapshot(exp),marker=manifest(exp);if(hasSource(exp)){const digest=String(exp.raw&&exp.raw.sha256||'')||await sha256Hex(sourceArchive(exp));if(!digest)throw new Error('Cannot create a portable LabFlow save without a RAW SHA-256 digest.');marker.source.sha256=digest;data.raw=data.raw||{};data.raw.sha256=digest;zip.file('raw/source.zip',sourceArchive(exp));}zip.file('labflow.json',JSON.stringify(marker,null,2));zip.file('experiment.json',JSON.stringify(data,null,2));return zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}});}
