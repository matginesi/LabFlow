(function () {
  ...
  function snapshot(exp){return LF.DomainSchema&&LF.DomainSchema.snapshot?LF.DomainSchema.snapshot(exp):JSON.parse(JSON.stringify(exp));}
  ...
  function canonicalSnapshot(exp){const store=LF.CanonicalStore&&LF.CanonicalStore.ensure?LF.CanonicalStore.ensure(exp):null;if(!store)return null;const copy=JSON.parse(JSON.stringify(store)),ms=copy.records&&copy.records.measurements||[];ms.forEach(function(m){delete m.curve;delete m.curves;delete m.points;});return copy;}
  ...
  function fileName(exp){return(C.safeName?C.safeName(exp&&exp.meta&&exp.meta.name||'experiment'):'experiment')+'_labflow.zip';}
