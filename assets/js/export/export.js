(function () {
  ...
  async function saveZip(exp){if(!window.JSZip)throw new Error('JSZip is unavailable.');const zip=new JSZip();zip.file('labflow.json',JSON.stringify(manifest(exp),null,2));const data=snapshot(exp);zip.file('experiment.json',JSON.stringify(data,null,2));if(hasSource(exp))zip.file('raw/source.zip',sourceArchive(exp));return zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}});}
