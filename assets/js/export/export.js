...
(function () {
  ...
  function manifest(exp){
    return{
      format:'labflow-save',formatVersion:2,generatedAt:new Date().toISOString(),experimentId:exp.id||'',
      workspaceId:exp.meta&&exp.meta.workspaceId||'',processId:exp.meta&&exp.meta.processId||'',
      name:exp.meta&&exp.meta.name||'',source:{
        name:exp.meta&&exp.meta.sourceName||'',sha256:exp.raw&&exp.raw.sha256||'',immutable:true,included:hasSource(exp)
      },revision:Number(exp.sync&&exp.sync.revision||0),patches:(exp.patches||[]).length
    };
  }
...
const zip=new JSZip(),data=snapshot(exp),marker=manifest(exp);
    ...
    if(LF.Workspace&&LF.Workspace.snapshot)zip.file('workspace.json',JSON.stringify(LF.Workspace.snapshot({includeContacts:false}),null,2));
