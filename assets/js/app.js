  ...
  function bindEvents(){
    ...
    document.addEventListener('click',async function(e){
      try {
        ...
        if(e.target.closest('#exportKb')){const payload=LF.KnowledgeBase.exportState();C.downloadBlob(C.textBlob(JSON.stringify(payload,null,2),'application/json;charset=utf-8'),'labflow-knowledge-base.json');return;}
    ...
    document.addEventListener('change',async function(e){if(e.target&&e.target.id==='kbImportFile'){const file=e.target.files&&e.target.files[0];if(!file)return;try{const text=await file.text(),out=LF.KnowledgeBase.importState(text,'merge');S.state.ui.settingsKnowledgeId='';render();LF.UI.toast('Imported '+out.imported+' Knowledge Base entr'+(out.imported===1?'y':'ies')+'.','success');}catch(err){LF.UI.toast('Knowledge Base import failed: '+(err.message||String(err)),'error');}finally{e.target.value='';}return;}if(e.target&&e.target.id==='cabinetImportFile'){const file=e.target.files&&e.target.files[0];if(!file)return;try{const text=await file.text(),out=LF.Cabinet.importState(text,'merge');S.state.ui.cabinetSelectedId=null;S.state.ui.cabinetKind='all';render();LF.UI.toast('Imported '+out.imported+' Cabinet resource'+(out.imported===1?'':'s')+'.','success');}catch(err){LF.UI.toast('Cabinet import failed: '+(err.message||String(err)),'error');}finally{e.target.value='';}}});
