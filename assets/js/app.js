(function () {
  ...
  function hydrateShell(page) {
    ...
    const width = ({workspace:"wide",project:"wide",cabinet:"wide",knowledge:"wide",robotics:"wide",tools:"wide",settings:"wide",documentation:"wide","ui-kit":"wide"})[page] || "standard";
  ...
  function pipelineInspectorMarkup(pipeline, settings) {
    ...
    const stepRows = asArray(pipeline.steps).map((item) => {
    ...
    return `<div class="pipeline-inspector-content stack">
      <div><div class="cluster"><span class="badge badge-accent">${esc(pipeline.id)}</span>${pipelineStateBadge(state)}${enabled ? '<span class="badge badge-success">Enabled</span>' : '<span class="badge">Disabled</span>'}${isDefault ? '<span class="badge badge-accent">Default</span>' : ""}</div><h2>${esc(pipeline.name)}</h2><p>${esc(pipeline.description || "No description supplied.")}</p></div>
      <div class="metadata-list"><div><span>Version</span><strong>${esc(pipeline.version || "—")}</strong></div><div><span>Domain</span><strong>${esc(pipeline.domain || pipeline.project_type || "Generic")}</strong></div><div><span>Schema</span><strong>${esc(pipeline.schema_version || "Navigation manifest")}</strong></div><div><span>Steps</span><strong>${asArray(pipeline.steps).length}</strong></div><div><span>Resources</span><strong>${resourceGroups.length ? resourceGroups.join(", ") : "Inline manifest"}</strong></div><div><span>Delivery</span><strong>${esc(compatibility.delivery || "Static bundled")}</strong></div></div>
      ...
      <div class="cluster"><a class="btn btn-primary" href="${pipeline.id === "chose" ? "project.html?project=PRJ-2026-014" : "index.html#project-portfolio"}">${icon("arrow")} Open workflow</a><a class="btn" href="${pipelineDocumentationLink(pipeline.id)}">${icon("book")} Documentation</a></div>
    </div>`;
  }

  function pipelineSettingsMarkup(settings, selectedPipelineId) {
    ...
    return `<section class="summary-strip" aria-label="Pipeline registry summary"><div class="summary-item metric-summary"><span>Registered</span><strong>${registry.length}</strong><small>checked-in manifests</small></div><div class="summary-item metric-summary"><span>Enabled</span><strong>${enabled.size}</strong><small>available this session</small></div><div class="summary-item metric-summary"><span>Default</span><strong class="kpi-value-compact">${esc(defaultName)}</strong><small>new project preference</small></div><div class="summary-item metric-summary"><span>Strict contracts</span><strong>${strictCount}</strong><small>schema-driven pipelines</small></div></section>
      <div class="pipeline-settings-layout"><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Pipeline registry</h3><small>Enable workflows, select the default and inspect their bundled YAML contracts</small></div><span class="badge badge-success">Local registry</span></div><div class="table-wrap pipeline-registry-table"><table class="table-dense"><thead><tr><th>Availability</th><th>Pipeline</th><th>Contract</th><th>Steps</th><th>Default</th><th><span class="sr-only">Inspect</span></th></tr></thead><tbody>${rows}</tbody></table></div><div class="panel-footer"><div class="notice"><div>${icon("info")}</div><div><strong>Session management only</strong><p>These controls change the current in-memory demonstration. Download settings.yaml to review a configuration copy; source pipeline YAML remains unchanged.</p></div></div></div></section><aside class="panel pipeline-settings-inspector"><div class="panel-header"><div><h3 class="mb-0">Pipeline contract</h3><small>Identity, resources, steps and completion state</small></div>${icon("layers")}</div><div class="panel-body" id="pipeline-inspector">${pipelineInspectorMarkup(selected, settings)}</div></aside></div>`;
  }
  ...
  function renderSettings() {
    ...
    const renderPipelineSettingsPane = () => {
      ...
      $$('[data-pipeline-inspect]', root).forEach((button) => button.addEventListener("click", () => {
        selectedPipelineId = button.dataset.pipelineInspect;
        renderPipelineSettingsPane();
      }));
    };
