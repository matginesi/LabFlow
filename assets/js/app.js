    ...
    try{
      ...
      if(aiConfiguredSilently()&&LF.ActionRunner){
        ...
        const enriched=await LF.ActionRunner.run('analysis.enrich',{
          ...
          onRequest:function(info){LF.UI.activityUpdate({progress:.63,progressLabel:'Waiting for model · target '+(info.targetTokens||'—')+' tok',request:JSON.stringify(info.request,null,2),requestIsJson:true,showAiTrace:true,details:{'AI checkpoint':info.stepId,'Output target':info.targetTokens?info.targetTokens+' tok':'—','Request ceiling':info.maxTokens?info.maxTokens+' tok':'—'}});},
          onProgress:function(p){const stream=p.content||p.reasoning||'',fine=LF.ActionUI&&LF.ActionUI.streamFraction?LF.ActionUI.streamFraction(p):{fraction:.3,tokens:Math.ceil(String(stream).length/4),target:Number(p.targetTokens)||Number(p.budgetTokens)||1,events:Number(p.meaningfulEvents)||p.events},shownTokens=Number.isFinite(Number(p.tokens))?Number(p.tokens):fine.tokens;const progress=.56+Math.min(.32,Math.max(.06,(fine.fraction-.14)*.47));LF.UI.activityUpdate({stage:fine.tokens?'Experiment brief · receiving model response':'Experiment brief · waiting for first token',progress:progress,progressLabel:'Streaming · '+shownTokens+' / ~'+fine.target+' tok',response:stream||'Waiting for model output…',responseIsJson:false,stream:{active:true,status:'streaming',events:fine.events,bytes:p.bytes,ttftMs:p.ttftMs,tokens:shownTokens,rate:p.rate,estimated:p.estimated!==false,budgetTokens:p.maxTokens||p.budgetTokens||null,targetTokens:p.targetTokens||null}});}
        });
  ...
  function bindEvents(){
    ...
    document.addEventListener('click',async function(e){
      try {
        ...
        if(e.target.closest('#resetAll')){if(!await LF.UI.confirmAction('The persisted Working Copy, action history, chat, report/design state and RAW snapshot will be cleared. The connected Knowledge Base folder, provider, API key and theme preferences are kept.',{title:'Reset current session',confirmLabel:'Reset session',danger:true}))return;S.resetSession();S.state.pceDistributionZoom=1;if(LF.Storage&&LF.Storage.clearSavedExperiment)await LF.Storage.clearSavedExperiment();if(LF.PageContext)LF.PageContext.clear();render();LF.UI.toast('Session reset. Ready for a new ZIP.','info');return;}
        ...
        if(e.target.closest('#knowledgeConnect')){const out=await LF.KnowledgeBase.connectDirectory();S.state.knowledgeCreating=false;S.state.knowledgeSelectedId=null;render();LF.UI.toast('Knowledge Base connected: '+out.folder+'/'+out.file+'.','success');return;}
        if(e.target.closest('#knowledgeChangeFolder')){const out=await LF.KnowledgeBase.connectDirectory({choose:true});S.state.knowledgeCreating=false;S.state.knowledgeSelectedId=null;render();LF.UI.toast('Knowledge Base folder changed to '+out.folder+'.','success');return;}
        ...
        if(e.target.closest('#knowledgeDisconnect')){await LF.KnowledgeBase.disconnect();S.state.knowledgeCreating=false;S.state.knowledgeSelectedId=null;render();LF.UI.toast('Knowledge Base folder disconnected. Files were not changed.','info');return;}
        ...
        if(e.target.closest('#applyAllDesignInferences')){const ok=await LF.UI.confirmAction('Apply every currently available AI Design inference to fields that are still empty? Existing ZIP and researcher-entered values will remain protected. Variants can remain incomplete after this operation.',{title:'Apply all Design inferences',confirmLabel:'Apply safe gaps',cancelLabel:'Cancel'});if(!ok)return;try{const out=LF.DesignAnalysis.applyAllProposals(S.state.experiment);if(out.changed){markModified('design');render();LF.UI.toast('Applied '+out.changed+' AI-proposed field'+(out.changed===1?'':'s')+' across '+out.proposals+' variant'+(out.proposals===1?'':'s')+'.','success');}else{render();LF.UI.toast('No still-empty Design field could be filled from the available proposals.','info');}}catch(err){LF.UI.toast(err.message||String(err),'error');}return;}
    ...
    document.addEventListener('submit',async function(e){
      if(e.target.id==='knowledgeEntryForm'){
        e.preventDefault();
        try{const record=await LF.KnowledgeBase.upsert(LF.KnowledgePage.readForm(e.target));S.state.knowledgeSelectedId=record.id;S.state.knowledgeCreating=false;render();LF.UI.toast('Knowledge record saved to library.json.','success');}catch(err){LF.UI.toast(err.message||String(err),'error');}
        return;
      }
