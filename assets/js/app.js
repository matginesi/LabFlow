(function () {
  ...
  const icons = {
    ...
    text: '<path d="M5 6h14M8 10h8M5 14h14M5 18h10"/>',
    markdown: '<path d="M3 5h18v14H3z"/><path d="M6 15V9l3 3 3-3v6M15 12l2 2 2-2M17 9v5"/>',
    formula: '<path d="M17 4H9L6 12l3 8h8M8 12h7"/>',
    braces: '<path d="M9 4H7a2 2 0 0 0-2 2v4l-2 2 2 2v4a2 2 0 0 0 2 2h2M15 4h2a2 2 0 0 1 2 2v4l2 2-2 2v4a2 2 0 0 1-2 2h-2"/>',
    hierarchy: '<rect x="9" y="3" width="6" height="5"/><rect x="3" y="16" width="6" height="5"/><rect x="15" y="16" width="6" height="5"/><path d="M12 8v4M6 16v-4h12v4"/>',
    diagram: '<rect x="3" y="4" width="7" height="5" rx="1"/><rect x="14" y="15" width="7" height="5" rx="1"/><path d="M10 6.5h4a3 3 0 0 1 3 3V15m0 0-3-3m3 3 3-3"/>',
    edit: '<path d="m4 20 4-1 11-11-3-3L5 16zM14 7l3 3"/>',
    eye: '<path d="M2 12s4-7 10-7 10 7 10 7-4 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/>',
    swap: '<path d="m7 7-4 4 4 4M3 11h14M17 17l4-4-4-4M21 13H7"/>',
    curve: '<path d="M3 18c5 0 4-12 9-12s4 12 9 12"/>',
  ...
  function icon(name, cls = "icon") {
    return `<svg class="${cls} icon-${name}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${name === "search" ? "2" : "1.8"}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${icons[name] || icons.info}</svg>`;
  }
  ...
  const defaultSettings = {
    ...
    allowedExports: C.exports?.enabled || ["pdf", "docx", "xlsx", "bundle"],
  ...
  function navigateWithSettings(href) {
    location.href = withCarriedSettings(href);
  }
  ...
  function renderWorkspace() {
    ...
    const actions = `<a class="btn" href="knowledge.html">${icon("spark")} Ask LabFlow</a><button class="btn btn-primary" id="new-project">${icon("plus")} New project</button>`;
    ...
    $("#page-content").innerHTML = header("Workspace", `Research control for ${esc(D.user.name)} · projects, evidence, quality and next decisions in one place.`, actions) + `
      <section class="summary-strip summary-strip-metrics" aria-label="Workspace summary">
        ${[["Projects", projects.length, `${active} active`], ["Research review", reviews, "projects need a decision"], ["Data health", "94%", "1 error · 2 warnings"], ["Export readiness", `${complete}/${projects.length}`, "complete or archived"]].map(([label, value, detail]) => `<div class="summary-item metric-summary"><span>${label}</span><strong>${value}</strong><small>${detail}</small></div>`).join("")}
      </section>
      ...
      <section class="section grid grid-2">
        <div class="panel"><div class="panel-header"><div><h3 class="mb-0">Laboratory knowledge</h3><small>Recently updated context</small></div><a class="btn btn-sm" href="knowledge.html">Open Knowledge</a></div><div class="panel-body stack">${D.knowledge.slice(0, 3).map(knowledgeCard).join("")}</div></div>
  ...
  function stackEditorRow(layer, index) {
    ...
    return `<div class="layer" data-layer data-tone="${esc(layer.tone || "interface")}"><span class="step-index">${String(index + 1).padStart(2,"0")}</span>${field("material","Material",layer.material)}${field("thickness","Thickness",layer.thickness)}${field("function","Function",layer.function)}${field("process","Process",layer.process)}<div class="layer-actions"><button class="btn btn-ghost icon-btn" data-layer-move="up" aria-label="Move layer up">↑</button><button class="btn btn-ghost icon-btn" data-layer-move="down" aria-label="Move layer down">↓</button><button class="btn btn-ghost icon-btn" data-layer-copy aria-label="Duplicate layer">${icon("copy")}</button><button class="btn btn-ghost icon-btn" data-layer-delete aria-label="Delete layer">${icon("trash")}</button></div></div>`;
  }
  ...
  function analysisStep(project) {
    return `<div class="panel"><div class="tabs" role="tablist"><button class="tab active" data-tab="overview">Overview</button><button class="tab" data-tab="inspect">Experiment Inspector</button><button class="tab" data-tab="compare">Comparison</button><button class="tab" data-tab="tools">Tools</button><button class="tab" data-tab="findings">Assistant findings</button><button class="tab" data-tab="report">Report</button></div><div class="panel-body" id="analysis-content">${analysisOverview(project)}</div></div>`;
  }
...
└── MANIFEST.txt`)}</pre></div></div><div class="panel ai-panel"><div class="panel-header"><div><h3 class="mb-0">NOMAD readiness</h3><small>Deterministic validation before packaging</small></div>${icon("spark")}</div><div class="panel-body stack"><div class="notice notice-success"><div>${icon("check")}</div><div><strong>Preview package can be generated</strong><p>Project, samples, results and evidence remain traceably connected.</p></div></div><div class="notice notice-warning"><div>${icon("warning")}</div><div><strong>Submission blocked by three issues</strong><p>Resolve device count, annealing unit and solution provenance before final submission.</p></div></div><a class="btn btn-secondary" href="knowledge.html?q=Inspect%20EXP-067%20for%20NOMAD">Inspect blocking issues</a></div></div><div class="notice"><div>${icon("lock")}</div><div><strong>No automatic upload</strong><p>Credentials, schema negotiation and remote submission are outside the current demonstration. LabFlow only creates transparent local packages.</p></div></div></div></div></div>`;
  ...
  function bindProjectInteractions(project, pipeline, step) {
    if (step.view === "solutions") {
      const recalculate = () => {
        ...
        $("#fai-mass").value = `${(146.1 * volume * molarity / 1.25).toFixed(1)} mg`;
        $("#mai-mass").value = `${(15.9 * volume * molarity / 1.25).toFixed(1)} mg`;
        $("#pbi-mass").value = `${(461.0 * volume * molarity / 1.25).toFixed(1)} mg`;
        $("[data-solution-concentration]").textContent = `${molarity.toFixed(2)} M`;
        $("[data-solution-volume]").textContent = `${volume.toFixed(2)} mL`;
        [["fai", "#fai-mass"], ["mai", "#mai-mass"], ["pbi", "#pbi-mass"]].forEach(([tone, selector]) => { $(`[data-solution-component="${tone}"] [data-component-amount]`).textContent = $(selector).value; });
      };
      $("#recalculate")?.addEventListener("click", () => {
        recalculate();
        toast("Solution masses recalculated.");
      });
    ...
    if (step.view === "analysis") {
      ...
      const requestedTab = ["overview", "inspect", "compare", "tools", "findings", "report"].includes(requestedView) ? $(`[data-tab="${requestedView}"]`) : null;
      if (requestedTab) requestedTab.click(); else bindAnalysisChartControls();
    }
    $$('[data-export]').forEach((button) => button.addEventListener("click", () => runExport(button.dataset.export, project, pipeline)));
  }
  ...
  function bindAnalysisTabs(project) {
    $$(".tab").forEach((tab) => tab.addEventListener("click", () => {
      ...
      content.innerHTML = target === "overview" ? analysisOverview(project) : target === "inspect" ? experimentInspector() : target === "compare" ? comparisonView() : target === "tools" ? analysisTools() : target === "findings" ? analysisFindings() : analysisReport(project);
      ...
      $$('[data-export]', content).forEach((button) => button.addEventListener("click", () => runExport(button.dataset.export, project, P[project.pipeline])));
    }));
  }
  ...
  function runExport(type, project, pipeline) {
    const settings = getSettings();
    const report = reportState(project);
    ...
    try {
      ...
      if (type === "bundle" || type === "nomad") E.download(E.bundle(exportProject, pipeline, D.demoDataset, type === "nomad", options), `${base}-${type === "nomad" ? "nomad-preview" : "complete-project"}.zip`);
  ...
  function renderDocumentation() {
    ...
    if (!selected) {
      console.warn("[LabFlow Documentation] Requested document was not found.", {documentId:requested});
    ...
    if (!String(selected.markdown || "").trim()) {
      console.info("[LabFlow Documentation] Selected document is empty.", {documentId:selected.id});
    ...
    try {
      ...
      console.error("[LabFlow Documentation] Markdown rendering failed.", {documentId:selected.id, message:error.message});
    ...
    if (rendered.warnings.length) console.info("[LabFlow Documentation] Content rendered with recoverable warnings.", {documentId:selected.id, warnings:rendered.warnings});
  ...
  function renderUiKit() {
    ...
    $$('[data-export]').forEach((button) => button.addEventListener("click", () => runExport(button.dataset.export, D.projects[0], P.chose)));
