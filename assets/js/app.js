    ...
    try{
      ...
      if(aiConfiguredSilently()&&LF.ActionRunner){
        ...
        const enriched=await LF.ActionRunner.run('analysis.enrich',{
          ...
          onProgress:function(p){const stream=p.content||p.reasoning||'',fine=LF.ActionUI&&LF.ActionUI.streamFraction?LF.ActionUI.streamFraction(p):{fraction:.3,tokens:Math.ceil(String(stream).length/4),target:Number(p.targetTokens)||Number(p.budgetTokens)||1,events:Number(p.meaningfulEvents)||p.events};const progress=.56+Math.min(.32,Math.max(.06,(fine.fraction-.14)*.47));LF.UI.activityUpdate({stage:fine.tokens?'Experiment brief · receiving model response':'Experiment brief · waiting for first token',progress:progress,progressLabel:'Streaming · '+fine.tokens+' / ~'+fine.target+' tok',response:stream||'Waiting for model output…',responseIsJson:false,stream:{active:true,status:'streaming',events:fine.events,bytes:p.bytes,ttftMs:p.ttftMs,tokens:fine.tokens,budgetTokens:p.maxTokens||p.budgetTokens||null,targetTokens:p.targetTokens||null}});}
        });
