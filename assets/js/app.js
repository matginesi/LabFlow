(function () {
  ...
  const defaultSettings = {
    ...
    defaultPipeline: C.pipelines?.default || "chose",
  ...
  function getSettings() {
    ...
    const registeredIds = Object.keys(P || {}).filter((id) => P?.[id]?.id && Array.isArray(P[id].steps));
    const configuredIds = asArray(value.enabledPipelines).filter((id) => registeredIds.includes(id));
    value.enabledPipelines = configuredIds.length ? [...new Set(configuredIds)] : registeredIds.slice(0, 1);
    if (!registeredIds.includes(value.defaultPipeline) || !value.enabledPipelines.includes(value.defaultPipeline)) {
      const configuredDefault = C.pipelines?.default;
      value.defaultPipeline = value.enabledPipelines.includes(configuredDefault) ? configuredDefault : value.enabledPipelines[0] || "";
    }
    return value;
  }
  ...
  function pageMeta(page) {
    return ({
      ...
      "pipeline-studio": ["Pipeline Studio", "Local pipeline contract drafts and validation preview"],
  ...
  function hydrateShell(page) {
    ...
    const width = ({workspace:"wide",project:"wide",cabinet:"wide",knowledge:"wide",robotics:"wide",tools:"wide",settings:"wide","pipeline-studio":"wide",documentation:"wide","ui-kit":"wide"})[page] || "standard";
  ...
  function globalSearchItems() {
    ...
    add("Platform configuration", "Pipeline Studio", "Inspect and edit local pipeline YAML drafts", "pipeline-studio.html", "code");
  ...
  function renderWorkspace() {
    ...
    $("#page-content").innerHTML = header("Workspace", `Research control for ${esc(D.user.name)} · projects, evidence, quality and next decisions in one place.`, actions) + `
      ...
      <section class="panel section workspace-ai-foundation"><div class="panel-header"><div><span class="page-eyebrow">AI readiness</span><h2 class="mb-0">A structured, traceable foundation for scientific AI</h2><small>LabFlow prepares structured, traceable data without changing the researcher’s normal project workflow.</small></div><a class="btn btn-primary" href="knowledge.html?view=datasets">Inspect AI readiness</a></div><div class="panel-body stack"><div class="summary-strip summary-strip-three"><div class="summary-item metric-summary"><span>Overall readiness</span><strong>${readinessOverall}%</strong><small>${esc(F.readiness.status)}</small></div><div class="summary-item metric-summary"><span>Dataset snapshots</span><strong>${F.datasetSnapshots.length}</strong><small>immutable versions</small></div><div class="summary-item metric-summary"><span>Reviewed predictions</span><strong>${F.predictions.filter((item) => item.status === "reviewed").length}</strong><small>human-controlled outputs</small></div></div><div class="grid grid-2"><div class="stack">${F.readiness.metrics.slice(0,4).map((item) => `<article class="ai-progress-row"><div><strong>${esc(item.label)}</strong><span>${item.value}%</span></div><div class="progress"><span style="width:${item.value}%"></span></div><small>${esc(item.detail)}</small></article>`).join("")}</div><div class="metadata-list"><div><span>Knowledge</span><strong>Evidence-led answers</strong></div><div><span>Datasets</span><strong>Snapshots, schema and quality</strong></div><div><span>Models</span><strong>Runs, metrics and provenance</strong></div><div><span>Predictions</span><strong>Uncertainty and human review</strong></div></div></div></div></section>
  ...
  function openNewProject() {
    const settings = getSettings();
    const enabledPipelines = asArray(settings.enabledPipelines).map((id) => P?.[id]).filter(Boolean);
    const pipelines = enabledPipelines.length ? enabledPipelines : Object.values(P || {}).filter((pipeline) => pipeline?.id);
    const defaultPipelineId = pipelines.some((pipeline) => pipeline.id === settings.defaultPipeline) ? settings.defaultPipeline : pipelines[0]?.id || "";
    modal(`<div class="modal" role="dialog" aria-modal="true" aria-labelledby="new-project-title">
      <div class="modal-header"><div><h2 id="new-project-title" class="mb-0">Create project</h2><small>Choose an enabled reusable pipeline and define the research decision.</small></div><button class="btn btn-ghost icon-btn" data-action="close-modal">${icon("x")}</button></div>
      <form id="new-project-form"><div class="modal-body stack">
        ...
        <div class="field"><label for="project-pipeline">Pipeline</label><select class="select" id="project-pipeline" ${pipelines.length ? "" : "disabled"}>${pipelines.map((pipeline) => `<option value="${pipeline.id}" ${pipeline.id === defaultPipelineId ? "selected" : ""}>${esc(pipeline.name)} · ${pipeline.steps.length} steps</option>`).join("")}</select><span class="field-help">Availability and the default are controlled in Settings → Pipelines. All steps remain revisitable and exports preserve the pipeline definition.</span></div>
        ${pipelines.length ? "" : `<div class="notice notice-warning"><div>${icon("warning")}</div><div><strong>No pipeline is available</strong><p>Open Settings → Pipelines and enable at least one checked-in workflow.</p></div></div>`}
      </div><div class="modal-footer"><button type="button" class="btn" data-action="close-modal">Cancel</button><button class="btn btn-primary" ${pipelines.length ? "" : "disabled"}>Create project</button></div></form>
    </div>`);
  ...
  function stackReview(layers = stackLayers, selectable = true) {
    if (!asArray(layers).length) return `<div class="empty">${icon("layers")}<strong>No stack layers declared</strong><p>Add fabrication producers or a stack definition to the CHOSE pipeline resources.</p></div>`;
  ...
  function pipelineRegistry() {
    return Object.values(P || {}).filter((item) => item && item.id && Array.isArray(item.steps));
  }

  function pipelineContractState(pipeline) {
    const strict = Boolean(pipeline?.contract?.strict);
    const evaluations = strict && R?.evaluateStep
      ? asArray(pipeline.steps).map((item) => R.evaluateStep(pipeline, item.id))
      : [];
    const errors = evaluations.reduce((total, item) => total + Number(item.errors || 0), 0);
    const warnings = evaluations.reduce((total, item) => total + Number(item.warnings || 0), 0);
    const ready = strict && evaluations.length > 0 && evaluations.every((item) => item.ready);
    return {
      strict,
      evaluations,
      errors,
      warnings,
      status: !strict ? "manifest" : errors ? "blocked" : warnings ? "warning" : ready ? "ready" : "review"
    };
  }

  function pipelineStateBadge(state) {
    const labels = {ready:"Contract ready",warning:"Warnings",blocked:"Blocked",review:"Review",manifest:"Navigation manifest"};
    const classes = {ready:"badge-success",warning:"badge-warning",blocked:"badge-danger",review:"badge-warning",manifest:""};
    return `<span class="badge ${classes[state.status] || ""}">${labels[state.status] || "Available"}</span>`;
  }

  function pipelineDocumentationLink(pipelineId) {
    const doc = pipelineId === "chose" ? "pipeline-chose" : pipelineId === "quick" ? "pipeline-quick" : "pipeline-catalog";
    return `documentation.html?doc=${encodeURIComponent(doc)}`;
  }

  function pipelineDraftSteps(settings, pipeline) {
    const draft = settings?.stepDrafts?.[pipeline?.id];
    return Array.isArray(draft) && draft.length ? draft : null;
  }

  function pipelineInspectorMarkup(pipeline, settings) {
    if (!pipeline) return `<div class="empty"><strong>No pipeline selected</strong><p>Select a registered pipeline to inspect its contract.</p></div>`;
    const enabled = asArray(settings.enabledPipelines).includes(pipeline.id);
    const isDefault = settings.defaultPipeline === pipeline.id;
    const state = pipelineContractState(pipeline);
    const resourceGroups = Object.keys(pipeline.resources || {});
    const compatibility = pipeline.compatibility || {};
    const steps = pipelineDraftSteps(settings, pipeline) || asArray(pipeline.steps);
    const stepRows = steps.map((item) => {
      const evaluation = state.evaluations.find((entry) => entry.step === item.id);
      const status = evaluation
        ? `<span class="badge ${evaluation.status === "ready" ? "badge-success" : evaluation.status === "warning" ? "badge-warning" : "badge-danger"}">${esc(evaluation.status)}</span>`
        : `<span class="badge">Defined</span>`;
      return `<tr><td><strong>${esc(item.short_title || item.title)}</strong><small class="block"><code>${esc(item.id)}</code></small></td><td>${esc(item.output || "Defined output")}</td><td>${status}</td></tr>`;
    }).join("");
    return `<div class="pipeline-inspector-content stack">
      <div class="entity-heading"><div class="cluster"><span class="badge badge-accent">${esc(pipeline.id)}</span>${pipelineStateBadge(state)}${enabled ? '<span class="badge badge-success">Enabled</span>' : '<span class="badge">Disabled</span>'}${isDefault ? '<span class="badge badge-accent">Default</span>' : ""}</div><strong class="entity-title">${esc(pipeline.name)}</strong><p class="entity-description">${esc(pipeline.description || "No description supplied.")}</p></div>
      <div class="metadata-list"><div><span>Version</span><strong>${esc(pipeline.version || "—")}</strong></div><div><span>Domain</span><strong>${esc(pipeline.domain || pipeline.project_type || "Generic")}</strong></div><div><span>Schema</span><strong>${esc(pipeline.schema_version || "Navigation manifest")}</strong></div><div><span>Steps</span><strong>${steps.length}</strong></div><div><span>Resources</span><strong>${resourceGroups.length ? resourceGroups.join(", ") : "Inline manifest"}</strong></div><div><span>Delivery</span><strong>${esc(compatibility.delivery || "Static bundled")}</strong></div></div>
      <div class="table-wrap"><table class="table-dense"><thead><tr><th>Step</th><th>Output</th><th>Contract state</th></tr></thead><tbody>${stepRows}</tbody></table></div>
      ${state.strict ? `<div class="notice ${state.errors ? "notice-warning" : "notice-success"}"><div>${icon(state.errors ? "warning" : "check")}</div><div><strong>${state.errors ? `${state.errors} contract error${state.errors === 1 ? "" : "s"}` : "Strict YAML contract resolved"}</strong><p>${state.warnings} warning${state.warnings === 1 ? "" : "s"} across ${state.evaluations.length} dependent completion gate${state.evaluations.length === 1 ? "" : "s"}.</p></div></div>` : `<div class="notice"><div>${icon("info")}</div><div><strong>Lightweight example pipeline</strong><p>This pipeline defines navigation and shared views but does not yet declare the strict schemas, resources and completion gates used by CHOSE.</p></div></div>`}
      <div class="cluster"><a class="btn btn-primary" href="pipeline-studio.html?pipeline=${encodeURIComponent(pipeline.id)}">${icon("code")} Open Pipeline Studio</a><a class="btn" href="${pipeline.id === "chose" ? "project.html?project=PRJ-2026-014" : "index.html#project-portfolio"}">${icon("arrow")} Open workflow</a><a class="btn" href="${pipelineDocumentationLink(pipeline.id)}">${icon("book")} Documentation</a></div>
    </div>`;
  }

  function pipelineStepEditorMarkup(settings, pipeline) {
    if (!pipeline || !settings.stepDrafts) return "";
    const steps = pipelineDraftSteps(settings, pipeline);
    if (!steps) return "";
    const rows = steps.map((step, index) => `
      <div class="pipeline-step-row" data-step-id="${esc(step.id)}">
        <span class="pipeline-step-index">${index + 1}</span>
        <span class="pipeline-step-move">
          <button type="button" class="icon-btn btn-ghost" data-step-move="-1" aria-label="Move step up" title="Move up" ${index === 0 ? "disabled" : ""}>${icon("chevron-up")}</button>
          <button type="button" class="icon-btn btn-ghost" data-step-move="1" aria-label="Move step down" title="Move down" ${index === steps.length - 1 ? "disabled" : ""}>${icon("chevron-down")}</button>
        </span>
        <span class="pipeline-step-field">
          <input class="input" data-step-field="title" value="${esc(step.title)}" aria-label="Step ${index + 1} title">
          <small><code>${esc(step.id)}</code></small>
        </span>
        <input class="input pipeline-step-output" data-step-field="output" value="${esc(step.output || "")}" aria-label="Step ${index + 1} output">
        <button type="button" class="icon-btn btn-ghost" data-step-remove aria-label="Remove step" title="Remove">${icon("trash")}</button>
      </div>`).join("");
    return `<section class="panel step-editor"><div class="panel-header"><div><h3 class="mb-0">Workflow step editor</h3><small>Reorder, relabel and add steps for this pipeline. Changes stay in session memory and are never written back to the checked-in YAML.</small></div><span class="badge badge-accent">Session draft</span></div><div class="panel-body stack"><div class="step-editor-list">${rows}</div><div class="cluster"><button type="button" class="btn btn-sm" data-step-add>${icon("plus")} Add step</button><button type="button" class="btn btn-sm btn-secondary" data-step-reset>${icon("swap")} Reset to source</button><span class="badge">${steps.length} step${steps.length === 1 ? "" : "s"}</span></div></div></section>`;
  }

  function pipelineSettingsMarkup(settings, selectedPipelineId) {    const registry = pipelineRegistry();
    const enabled = new Set(asArray(settings.enabledPipelines).filter((id) => P?.[id]));
    const selected = P?.[selectedPipelineId] || P?.[settings.defaultPipeline] || registry[0];
    const strictCount = registry.filter((item) => item.contract?.strict).length;
    const defaultPipeline = P?.[settings.defaultPipeline];
    const defaultName = defaultPipeline?.short_name || String(defaultPipeline?.id || "Not selected").toUpperCase();
    const rows = registry.map((pipeline) => {
      const state = pipelineContractState(pipeline);
      const active = enabled.has(pipeline.id);
      const isDefault = settings.defaultPipeline === pipeline.id;
      return `<tr class="${selected?.id === pipeline.id ? "selected-row" : ""}"><td><label class="toggle pipeline-toggle"><input type="checkbox" data-pipeline-toggle="${esc(pipeline.id)}" ${active ? "checked" : ""}><span class="toggle-track"></span><span>${active ? "Enabled" : "Disabled"}</span></label></td><td><strong>${esc(pipeline.name)}</strong><small class="block"><code>${esc(pipeline.id)}</code> · v${esc(pipeline.version || "—")}</small></td><td>${pipelineStateBadge(state)}</td><td>${asArray(pipeline.steps).length}</td><td><label class="pipeline-default-choice"><input type="radio" name="default-pipeline" value="${esc(pipeline.id)}" ${isDefault ? "checked" : ""} ${active ? "" : "disabled"}><span>${isDefault ? "Default" : "Set default"}</span></label></td><td><div class="cluster pipeline-row-actions"><button class="btn btn-sm" type="button" data-pipeline-preview="${esc(pipeline.id)}">Preview</button><a class="btn btn-sm btn-secondary" href="pipeline-studio.html?pipeline=${encodeURIComponent(pipeline.id)}">Edit draft</a></div></td></tr>`;
    }).join("");
    return `<section class="summary-strip" aria-label="Pipeline registry summary"><div class="summary-item metric-summary"><span>Registered</span><strong>${registry.length}</strong><small>checked-in manifests</small></div><div class="summary-item metric-summary"><span>Enabled</span><strong>${enabled.size}</strong><small>available this session</small></div><div class="summary-item metric-summary"><span>Default</span><strong class="kpi-value-compact">${esc(defaultName)}</strong><small>new project preference</small></div><div class="summary-item metric-summary"><span>Strict contracts</span><strong>${strictCount}</strong><small>schema-driven pipelines</small></div></section>
      <div class="pipeline-settings-layout"><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Pipeline registry</h3><small>Enable workflows, select the default, preview contracts and open local YAML drafts</small></div><span class="badge badge-success">Local registry</span></div><div class="table-wrap pipeline-registry-table"><table class="table-dense"><thead><tr><th>Availability</th><th>Pipeline</th><th>Contract</th><th>Steps</th><th>Default</th><th>Actions</th></tr></thead><tbody>${rows}</tbody></table></div><div class="panel-footer"><div class="notice"><div>${icon("info")}</div><div><strong>Session management only</strong><p>These controls change the current in-memory demonstration. Download settings.yaml to review a configuration copy; source pipeline YAML remains unchanged.</p></div></div></div></section><aside class="panel pipeline-settings-inspector"><div class="panel-header"><div><h3 class="mb-0">Pipeline contract</h3><small>Identity, resources, steps and completion state</small></div>${icon("layers")}</div><div class="panel-body stack"><div id="pipeline-step-editor">${pipelineStepEditorMarkup(settings, selected)}</div><div id="pipeline-inspector">${pipelineInspectorMarkup(selected, settings)}</div></div></aside></div>`;
  }
  ...
  function renderSettings() {
    ...
    const registeredPipelines = pipelineRegistry();
    const initialEnabledPipelines = asArray(settings.enabledPipelines).filter((id) => P?.[id]);
    const pipelineDraft = {
      enabledPipelines: initialEnabledPipelines.length ? [...new Set(initialEnabledPipelines)] : registeredPipelines.slice(0, 1).map((item) => item.id),
      defaultPipeline: settings.defaultPipeline && P?.[settings.defaultPipeline] ? settings.defaultPipeline : C.pipelines?.default || registeredPipelines[0]?.id || "",
      stepDrafts: {},
      customSeq: 0
    };
    if (!pipelineDraft.enabledPipelines.includes(pipelineDraft.defaultPipeline)) pipelineDraft.defaultPipeline = pipelineDraft.enabledPipelines[0] || "";
    let selectedPipelineId = pipelineDraft.defaultPipeline || registeredPipelines[0]?.id || "";
    $("#page-content").innerHTML = header("Settings", "Manage profile, workspace access, pipeline availability, appearance and local integration policies from one wide configuration workspace.", `<button class="btn" id="download-settings">${icon("download")} Download settings.yaml</button><button class="btn btn-primary" id="save-settings">${icon("check")} Apply for this session</button>`) + `
      ...
      <div id="pipelines-settings" class="settings-pane stack" role="tabpanel" hidden><h2 class="sr-only">Pipeline settings</h2><div id="pipelines-settings-content"></div></div>
    ...
    $("#download-settings").onclick = () => {
      ...
      copy.pipelines = {...copy.pipelines, enabled:[...pipelineDraft.enabledPipelines], default:pipelineDraft.defaultPipeline};
    ...
    const ensurePipelineDraft = (pipeline) => {
      if (!pipeline) return;
      if (!pipelineDraft.stepDrafts[pipeline.id]) {
        pipelineDraft.stepDrafts[pipeline.id] = asArray(pipeline.steps).map((step) => ({id: step.id, title: step.title, short_title: step.short_title || step.title, output: step.output || ""}));
      }
    };
    const renderPipelineSettingsPane = () => {
      const root = $("#pipelines-settings-content");
      if (!root) return;
      const preview = P?.[selectedPipelineId] || P?.[pipelineDraft.defaultPipeline] || pipelineRegistry()[0];
      ensurePipelineDraft(preview);
      root.innerHTML = pipelineSettingsMarkup(pipelineDraft, selectedPipelineId);
      const applyStepDraft = (fn) => {
        const pipe = P?.[selectedPipelineId] || P?.[pipelineDraft.defaultPipeline] || pipelineRegistry()[0];
        if (!pipe) return;
        ensurePipelineDraft(pipe);
        fn(pipelineDraft.stepDrafts[pipe.id]);
        renderPipelineSettingsPane();
      };
      const currentPipe = () => P?.[selectedPipelineId] || P?.[pipelineDraft.defaultPipeline] || pipelineRegistry()[0];
      $$('[data-pipeline-toggle]', root).forEach((input) => input.addEventListener("change", () => {
        const next = new Set(pipelineDraft.enabledPipelines);
        input.checked ? next.add(input.dataset.pipelineToggle) : next.delete(input.dataset.pipelineToggle);
        if (!next.size) {
          input.checked = true;
          toast("At least one pipeline must remain enabled.", "error");
          return;
        }
        pipelineDraft.enabledPipelines = [...next];
        if (!next.has(pipelineDraft.defaultPipeline)) pipelineDraft.defaultPipeline = pipelineDraft.enabledPipelines[0];
        renderPipelineSettingsPane();
      }));
      $$('input[name="default-pipeline"]', root).forEach((input) => input.addEventListener("change", () => {
        if (!input.checked || !pipelineDraft.enabledPipelines.includes(input.value)) return;
        pipelineDraft.defaultPipeline = input.value;
        selectedPipelineId = input.value;
        renderPipelineSettingsPane();
      }));
      $$('[data-pipeline-preview]', root).forEach((button) => button.addEventListener("click", () => {
        selectedPipelineId = button.dataset.pipelinePreview;
        renderPipelineSettingsPane();
        const inspector = $("#pipeline-inspector");
        if (inspector && matchMedia("(max-width: 1180px)").matches) inspector.scrollIntoView({behavior:"smooth", block:"start"});
      }));
      $$('[data-step-move]', root).forEach((button) => button.addEventListener("click", () => applyStepDraft((steps) => {
        const index = steps.findIndex((step) => step.id === button.closest("[data-step-id]")?.dataset.stepId);
        const delta = Number(button.dataset.stepMove);
        const target = index + delta;
        if (index < 0 || target < 0 || target >= steps.length) return;
        [steps[index], steps[target]] = [steps[target], steps[index]];
      })));
      $$('[data-step-field]', root).forEach((input) => input.addEventListener("input", () => {
        const pipe = currentPipe();
        if (!pipe) return;
        const steps = pipelineDraft.stepDrafts[pipe.id];
        const step = steps && steps.find((item) => item.id === input.closest("[data-step-id]")?.dataset.stepId);
        if (!step) return;
        const key = input.dataset.stepField;
        step[key] = input.value;
        if (key === "title") step.short_title = input.value;
        const inspector = $("#pipeline-inspector");
        if (inspector) inspector.innerHTML = pipelineInspectorMarkup(pipe, pipelineDraft);
      }));
      $$('[data-step-remove]', root).forEach((button) => button.addEventListener("click", () => applyStepDraft((steps) => {
        const index = steps.findIndex((step) => step.id === button.closest("[data-step-id]")?.dataset.stepId);
        if (index >= 0 && steps.length > 1) steps.splice(index, 1);
      })));
      $$('[data-step-add]', root).forEach((button) => button.addEventListener("click", () => applyStepDraft((steps) => {
        steps.push({id: `custom-${++pipelineDraft.customSeq}`, title: "New step", short_title: "New step", output: "Defined output"});
      })));
      $$('[data-step-reset]', root).forEach((button) => button.addEventListener("click", () => {
        const pipe = currentPipe();
        if (!pipe) return;
        delete pipelineDraft.stepDrafts[pipe.id];
        renderPipelineSettingsPane();
      }));
    };
    const activateSettingsTab = (name) => {
      const selected = ["user", "users", "pipelines", "admin"].includes(name) ? name : "user";
      $("#user-settings").hidden = selected !== "user";
      $("#users-settings").hidden = selected !== "users";
      $("#pipelines-settings").hidden = selected !== "pipelines";
      ...
      if (selected === "pipelines") renderPipelineSettingsPane();
    };
    $$('[data-settings-tab]').forEach((tab) => tab.onclick = () => activateSettingsTab(tab.dataset.settingsTab));
    $$('[data-settings-tab-jump]').forEach((button) => button.onclick = () => activateSettingsTab(button.dataset.settingsTabJump));
  ...
  function renderUiKit() {
    ...
    $("#page-content").innerHTML = header("UI Kit", "The single source of truth for the compact application shell, components, scientific visuals, AI patterns and report identity.", `<a class="btn btn-primary" href="settings.html">Test themes</a>`) + `
      ...
      <section class="section"><div class="section-heading"><div><h2>Pipeline registry pattern</h2><p>Settings uses a canonical summary, dense registry table and one inspector. Availability and default selection remain separate from the pipeline YAML source.</p></div><a class="btn" href="settings.html?tab=pipelines">Open pipeline settings</a></div>${pipelineSettingsMarkup({...getSettings(), enabledPipelines:C.pipelines?.enabled || Object.keys(P || {}), defaultPipeline:C.pipelines?.default || "chose"}, C.pipelines?.default || "chose")}</section>
      <section class="section"><div class="section-heading"><div><h2>AI & Models foundation</h2><p>The product uses the same summary strips, panels, tables, progress rows, badges and notices defined here. Only charts and scientific image previews add domain-specific rendering.</p></div><a class="btn btn-primary" href="knowledge.html?view=datasets">Open AI & Models</a></div><div class="grid grid-4"><article class="card"><span class="page-eyebrow">Knowledge</span><h3>Evidence-led answers</h3><p>Scope, sources, relationships, tools and limitations stay visible.</p></article><article class="card"><span class="page-eyebrow">Datasets</span><h3>Immutable snapshots</h3><p>Features, targets, units, selection, exclusions and split policy.</p></article><article class="card"><span class="page-eyebrow">Models</span><h3>Training and evaluation</h3><p>Learning curves, baselines, grouped validation and versioned runs.</p></article><article class="card"><span class="page-eyebrow">Predictions</span><h3>Reviewed outputs</h3><p>Prediction, uncertainty, applicability, observed value and review state.</p></article></div><div class="summary-strip mt-2"><div class="summary-item metric-summary"><span>Readiness</span><strong>${readinessOverall}%</strong><small>${esc(F.readiness.status)}</small></div><div class="summary-item metric-summary"><span>Models</span><strong>${F.models.length}</strong><small>registered demonstrations</small></div><div class="summary-item metric-summary"><span>Runs</span><strong>${trainingRuns.length}</strong><small>versioned executions</small></div><div class="summary-item metric-summary"><span>Outputs</span><strong>${outputTypes.length}</strong><small>scientific classes</small></div></div><div class="grid grid-2 mt-2"><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Model comparison pattern</h3><small>Exact pattern used by AI & Models</small></div><span class="badge">Demonstration data</span></div><div class="panel-body stack">${modelComparisonItems.map((item, index) => `<article class="ai-comparison-row"><div><strong>${esc(item.name)}</strong><small>${item.mae == null ? "Baseline comparison" : `MAE ${Number(item.mae).toFixed(2)} · RMSE ${Number(item.rmse || 0).toFixed(2)}`}</small></div><div class="progress"><span style="width:${Math.max(4, Number(item.r2 || 0) * 100)}%"></span></div><strong>R² ${Number(item.r2 || 0).toFixed(2)}</strong>${index === modelComparisonItems.length - 1 ? '<span class="badge badge-success">Best</span>' : ""}</article>`).join("")}</div></section><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Training-run pattern</h3><small>Canonical dense table, badges and explicit provenance</small></div>${icon("activity")}</div><div class="table-wrap"><table class="table-dense"><thead><tr><th>Run</th><th>Model / dataset</th><th>Metric</th><th>Status</th></tr></thead><tbody>${trainingRuns.slice(0,4).map((run) => `<tr><td><code>${esc(run.id)}</code></td><td>${esc(run.model)}<small class="block">${esc(run.dataset)}</small></td><td><strong>${esc(run.bestMetric || "Review")}</strong></td><td><span class="badge ${run.status === "completed" ? "badge-success" : "badge-warning"}">${esc(run.status)}</span></td></tr>`).join("")}</tbody></table></div></section></div><div class="grid grid-2 mt-2"><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Readiness pattern</h3><small>Score is decomposed into inspectable checks</small></div><span class="badge badge-warning">${readinessOverall}%</span></div><div class="panel-body stack">${F.readiness.metrics.slice(0,4).map((item) => `<article class="ai-progress-row"><div><strong>${esc(item.label)}</strong><span>${item.value}%</span></div><div class="progress"><span style="width:${item.value}%"></span></div><small>${esc(item.detail)}</small></article>`).join("")}</div></section><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Scientific output classes</h3><small>Output identity remains textual and reviewable</small></div>${icon("layers")}</div><div class="table-wrap"><table class="table-dense"><thead><tr><th>Output</th><th>Initial state</th><th>Required evidence</th></tr></thead><tbody>${outputTypes.map((item) => `<tr><td><span class="output-type output-${esc(item.type)}">${esc(item.label)}</span></td><td>${esc(item.review)}</td><td>${esc(item.evidence)}</td></tr>`).join("")}</tbody></table></div></section></div></section>
      <section class="section"><div class="section-heading"><div><h2>AI specialized visual patterns</h2><p>Charts and scientific image previews are the only domain-specific AI surfaces. They stay inside the same panels, tables, metadata lists, badges and notices used everywhere else.</p></div><span class="badge badge-success">UI Kit governed</span></div><div class="grid grid-2"><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Training chart pattern</h3><small>Local SVG · explicit axes · separate legend · no decorative dashboard chrome</small></div><span class="badge">Demonstration data</span></div><div class="panel-body"><div class="ai-chart"><svg viewBox="0 0 620 210" role="img" aria-label="Example training and validation loss"><line x1="54" y1="24" x2="54" y2="174" class="ai-chart-grid"/><line x1="54" y1="174" x2="600" y2="174" class="ai-chart-grid"/><line x1="54" y1="124" x2="600" y2="124" class="ai-chart-grid"/><line x1="54" y1="74" x2="600" y2="74" class="ai-chart-grid"/><polyline points="54,48 145,77 236,104 327,128 418,145 509,155 600,162" class="ai-chart-line series-1"/><polyline points="54,58 145,88 236,114 327,137 418,148 509,151 600,149" class="ai-chart-line series-2"/><text x="54" y="194">Epoch 1</text><text x="550" y="194">Epoch 7</text></svg></div><div class="ai-chart-legend"><span><i class="series-1"></i>Training loss</span><span><i class="series-2"></i>Validation loss</span></div></div></section><section class="panel"><div class="panel-header"><div><h3 class="mb-0">Scientific vision review pattern</h3><small>Preview is subordinate to source identity, metadata and human review</small></div><span class="badge badge-warning">Review required</span></div><div class="panel-body grid grid-2 ai-vision-inspector"><div class="ai-vision-large"><svg viewBox="0 0 240 132" role="img" aria-label="Example local scientific image prediction preview"><rect width="240" height="132" class="ai-vision-base"/><path d="M0 33H240M0 66H240M0 99H240M60 0V132M120 0V132M180 0V132" class="ai-vision-grid"/><rect x="18" y="18" width="204" height="96" class="ai-vision-frame"/><path d="M28 28 67 54 89 47 121 81 149 73 211 110" class="ai-vision-signal"/></svg></div><div class="stack"><div class="metadata-list"><div><span>Record</span><strong>IMG-DEMO-04</strong></div><div><span>Proposal</span><strong>Crack candidate</strong></div><div><span>Score</span><strong>87%</strong></div><div><span>State</span><strong>Awaiting review</strong></div></div><div class="notice notice-warning"><div>${icon("warning")}</div><div><strong>Prediction, not measurement</strong><p>Keep the source image, proposed label and researcher decision distinct.</p></div></div></div></div></section></div></section>
  ...
  function assertPageDependencies(page) {
    const requirements = {
      ...
      "pipeline-studio": [[window.LabFlowPipelineStudio, "render", "assets/js/pipeline-studio.js"]]
    };
  ...
  function init() {
    ...
    try {
      ...
      hydrateShell(page);
      const renderers = {workspace: renderWorkspace, project: renderProject, cabinet: renderCabinet, knowledge: () => window.LabFlowKnowledgePages.renderBase({root: $("#page-content"), header, icon, esc, badgeStatus}), robotics: () => window.LabFlowRobotics.renderBase({root: $("#page-content"), header, icon, esc, toast}), tools: () => window.LabFlowToolsPage.render({root: $("#page-content"), header, icon, esc, toast, getSettings}), settings: renderSettings, "pipeline-studio": () => window.LabFlowPipelineStudio.render({root: $("#page-content"), header, icon, esc, toast, pipelines: P, sources: window.LabFlowPipelineSources || {}, runtime: R, download: E.download}), documentation: renderDocumentation, "ui-kit": renderUiKit};
