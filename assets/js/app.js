(function () {
  ...
  function bindEvents(){
    ...
    document.addEventListener('click',async function(e){
      try {
        ...
        if(e.target.closest('#applyAllDesignSuggestions')){try{const out=LF.DesignAnalysis.applyAll(S.state.experiment,S.state.ui.selectedDesignDeviceId);if(out.changed){markModified('design');render();LF.UI.toast('Applied '+out.changed+' AI-proposed missing field'+(out.changed===1?'':'s')+' to Design.','success');}else{render();LF.UI.toast('No missing field could be filled without overwriting existing values.','info');}}catch(err){LF.UI.toast(err.message||String(err),'error');}return;}
        if(e.target.closest('#acceptAllDesignInferences')){const exp=S.state.experiment,count=Object.keys(LF.ActionData?LF.ActionData.proposals(exp,'design.infer'):{}).length;if(!count){LF.UI.toast('No AI Design suggestions are waiting for acceptance.','info');return;}const ok=await LF.UI.confirmAction('Accept all '+count+' current AI Design suggestion'+(count===1?'':'s')+'? LabFlow fills only empty chemistry, architecture and process fields; existing researcher or source values stay unchanged. You can edit accepted values immediately afterwards.',{title:'Accept all Design suggestions',confirmLabel:'Accept all',cancelLabel:'Cancel'});if(!ok)return;try{const out=LF.DesignAnalysis.acceptAllProposals(exp);if(out.changed)markModified('design');else markModified('ai');(out.acceptedIds||[]).forEach(function(id){LF.ActionData.setStatus(exp,'design.infer',id,{state:'accepted',updatedAt:new Date().toISOString(),message:''});});(out.incomplete||[]).forEach(function(item){LF.ActionData.setStatus(exp,'design.infer',item.id,{state:'incomplete',updatedAt:new Date().toISOString(),message:'Still missing: '+(item.remaining||[]).join(', ')});});(out.failed||[]).forEach(function(item){LF.ActionData.setStatus(exp,'design.infer',item.id,{state:'error',updatedAt:new Date().toISOString(),message:item.message||'Accept failed'});});render();const issues=(out.incomplete||[]).length+(out.failed||[]).length,msg='Accepted '+out.accepted+' experiment suggestion'+(out.accepted===1?'':'s')+((out.incomplete||[]).length?' · '+out.incomplete.length+' still incomplete':'')+((out.failed||[]).length?' · '+out.failed.length+' failed':'')+'.';LF.UI.toast(msg,issues?'warning':'success');}catch(err){LF.UI.toast(err.message||String(err),'error');}return;}
        if(e.target.closest('#applyAcceptedDesignProposal')){try{const out=LF.DesignAnalysis.applyAccepted(S.state.experiment,S.state.ui.selectedDesignDeviceId);markModified('design');render();LF.UI.toast('Applied '+(out.solutions+out.devices)+' accepted Design item(s).','success');}catch(err){LF.UI.toast(err.message||String(err),'error');}return;}
        const applyDesignProposal=e.target.closest('[data-apply-design-proposal]');if(applyDesignProposal){try{const out=LF.DesignAnalysis.applyOne(S.state.experiment,applyDesignProposal.dataset.applyDesignProposal,Number(applyDesignProposal.dataset.proposalIndex),applyDesignProposal.dataset.proposalPart||'all',S.state.ui.selectedDesignDeviceId);markModified('design');render();LF.UI.toast('AI suggestion applied to '+out.changed+' missing field'+(out.changed===1?'':'s')+'.','success');}catch(err){LF.UI.toast(err.message||String(err),'error');}return;}
        const applyDesignDevice=e.target.closest('[data-apply-design-device]');if(applyDesignDevice){try{const out=LF.DesignAnalysis.applySelectedDevice(S.state.experiment,applyDesignDevice.dataset.applyDesignDevice,S.state.ui.selectedDesignDeviceId);markModified('design');render();LF.UI.toast('Applied AI-suggested missing values to '+out.changed+' Design field(s) for the selected experiment.','success');}catch(err){LF.UI.toast(err.message||String(err),'error');}return;}
        const acceptDesignExperiment=e.target.closest('[data-accept-design-experiment]');if(acceptDesignExperiment){try{const exp=S.state.experiment,out=LF.DesignAnalysis.acceptProposal(exp,acceptDesignExperiment.dataset.acceptDesignExperiment);if(out.changed)markModified('design');else markModified('ai');LF.ActionData.setStatus(exp,'design.infer',out.deviceId,{state:out.complete?'accepted':'incomplete',updatedAt:new Date().toISOString(),message:out.complete?'':'Still missing: '+(out.remaining||[]).join(', ')});render();LF.UI.toast(out.complete?'AI suggestion accepted. The experiment Design is complete.':'AI values were accepted, but this older suggestion did not cover: '+(out.remaining||[]).join(', ')+'. Run Complete with AI once more for the remaining domains.',out.complete?'success':'warning');}catch(err){LF.UI.toast(err.message||String(err),'error');}return;}
        ...
        const useCabinetItem=e.target.closest('[data-use-cabinet-item]');if(useCabinetItem){
          try{
            ...
            const out=LF.Cabinet.applyToDesign(exp,dev.id,item.id,{replace:true});if(out.changed){S.state.ui.selectedDesignDeviceId=dev.id;markModified('design');S.state.ui.designCabinetPicker='';render();LF.UI.toast('Applied Cabinet snapshot: '+item.name+'.','success');}else LF.UI.toast('The selected Cabinet resource adds no new Design values.','info');
        ...
        if(e.target.closest('#addDeviceLayer')){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;});if(dev){dev.stack.push({id:C.uid('layer'),role:'',material:'',thickness:'',process:'',evidence:'User entry',status:'user_confirmed'});dev.status='user_confirmed';markModified('design');render();}return;}
        const removeDeviceLayer=e.target.closest('[data-remove-device-layer]');if(removeDeviceLayer){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;});if(dev){dev.stack.splice(Number(removeDeviceLayer.dataset.removeDeviceLayer),1);dev.status='user_confirmed';markModified('design');render();}return;}
        ...
        if(e.target.closest('#discardDesignProposal')){const exp=S.state.experiment,id=S.state.ui.selectedDesignDeviceId;if(LF.ActionData&&id)LF.ActionData.removeProposal(exp,'design.infer',id);Log.info('design.proposal-discarded',{deviceId:id||''});markModified('ai');render();LF.UI.toast('AI design proposal discarded.','info');return;}
        ...
        if(e.target.closest('#saveKbEntry')){const item=LF.KnowledgeBase.save(knowledgeFormEntry());S.state.ui.settingsKnowledgeId=item.id;LF.UI.toast(item.status==='active'?'Knowledge saved and available to AI.':'Knowledge draft saved.','success');render();return;}
        if(e.target.closest('#duplicateKbEntry')){const btn=e.target.closest('#duplicateKbEntry'),id=btn.dataset.kbId||S.state.ui.settingsKnowledgeId,item=LF.KnowledgeBase.duplicate(id);S.state.ui.settingsKnowledgeId=item.id;LF.UI.toast('Knowledge copied as a draft.','success');render();return;}
        if(e.target.closest('#deleteKbEntry')){const btn=e.target.closest('#deleteKbEntry'),id=btn.dataset.kbId||S.state.ui.settingsKnowledgeId,item=LF.KnowledgeBase.get(id);if(item&&await LF.UI.confirmAction('Delete “'+item.title+'” from the custom Knowledge Base?',{title:'Delete knowledge entry',confirmLabel:'Delete',danger:true})){LF.KnowledgeBase.remove(id);S.state.ui.settingsKnowledgeId='';LF.UI.toast('Knowledge entry deleted.','success');render();}return;}
        ...
        if(e.target.closest('#resetKbCustom')){if(await LF.UI.confirmAction('Clear every browser-local Knowledge Base entry? Export a backup first if you need to keep them.',{title:'Clear custom Knowledge Base',confirmLabel:'Clear custom KB',danger:true})){LF.KnowledgeBase.resetCustom();S.state.ui.settingsKnowledgeId='';LF.UI.toast('Custom Knowledge Base cleared.','success');render();}return;}
    ...
    document.addEventListener('change',function(e){
      try {
        ...
        const deviceSolution=e.target.closest('[data-device-solution-id]');if(deviceSolution){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;});if(dev){const id=deviceSolution.dataset.deviceSolutionId;dev.solutionIds=dev.solutionIds||[];if(deviceSolution.checked&&!dev.solutionIds.includes(id))dev.solutionIds.push(id);if(!deviceSolution.checked)dev.solutionIds=dev.solutionIds.filter(function(x){return x!==id;});dev.status='user_confirmed';markDraft('design');refreshDesignProjection();}return;}
        const deviceSample=e.target.closest('[data-device-sample-name]');if(deviceSample){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;}),name=deviceSample.dataset.deviceSampleName;if(dev){dev.sampleNames=dev.sampleNames||[];if(deviceSample.checked){d.devices.forEach(function(other){if(other.id!==dev.id)other.sampleNames=(other.sampleNames||[]).filter(function(x){return x!==name;});});if(!dev.sampleNames.includes(name))dev.sampleNames.push(name);d.devices=d.devices.filter(function(other){return other.id===dev.id||other.status!=='raw_evidence'||(other.sampleNames||[]).length>0;});}else dev.sampleNames=dev.sampleNames.filter(function(x){return x!==name;});dev.status='user_confirmed';markModified('design');render();}return;}
        if(e.target.id==='confirmSelectedDevice'){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;});if(dev){dev.status=e.target.checked?'user_confirmed':'raw_evidence';markModified('design');render();}return;}
    ...
    document.addEventListener('input',function(e){
      try {
        ...
        const deviceField=e.target.closest('[data-device-field]');if(deviceField){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;});if(dev){dev[deviceField.dataset.deviceField]=deviceField.value;dev.status='user_confirmed';markDraft('design');refreshDesignProjection();}return;}
        const deviceLayerField=e.target.closest('[data-device-layer-field]');if(deviceLayerField){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;}),item=dev&&dev.stack[Number(deviceLayerField.dataset.deviceLayerIndex)];if(item){item[deviceLayerField.dataset.deviceLayerField]=deviceLayerField.value;item.status='user_confirmed';dev.status='user_confirmed';markDraft('design');refreshDesignProjection();}return;}
        const deviceProcessField=e.target.closest('[data-device-process-field]');if(deviceProcessField){const d=ensureExperimentShape(S.state.experiment).design,dev=d.devices.find(function(x){return x.id===S.state.ui.selectedDesignDeviceId;});if(dev){const field=deviceProcessField.dataset.deviceProcessField;dev.process=dev.process||{};dev.process[field]=deviceProcessField.value;dev.processProvenance=dev.processProvenance||{};dev.processProvenance[field]={status:'user_confirmed',evidence:'User entry'};dev.status='user_confirmed';markDraft('design');refreshDesignProjection();}return;}
    ...
    document.addEventListener('change',async function(e){if(e.target&&e.target.id==='kbImportFile'){const file=e.target.files&&e.target.files[0];if(!file)return;try{const text=await file.text(),out=LF.KnowledgeBase.importJsonl(text,'merge');S.state.ui.settingsKnowledgeId='';render();LF.UI.toast('Imported '+out.imported+' Knowledge Base entr'+(out.imported===1?'y':'ies')+' from JSONL.','success');}catch(err){LF.UI.toast('Knowledge Base import failed: '+(err.message||String(err)),'error');}finally{e.target.value='';}return;}if(e.target&&e.target.id==='cabinetImportFile'){const file=e.target.files&&e.target.files[0];if(!file)return;try{const text=await file.text(),out=LF.Cabinet.importState(text,'merge');S.state.ui.cabinetSelectedId=null;S.state.ui.cabinetKind='all';render();LF.UI.toast('Imported '+out.imported+' Cabinet resource'+(out.imported===1?'':'s')+'.','success');}catch(err){LF.UI.toast('Cabinet import failed: '+(err.message||String(err)),'error');}finally{e.target.value='';}}});
  ...
  async function init(){
    ...
    try{
      S.state.route='experiment-import';S.state.ui.assistantOpen=window.innerWidth>1100&&LF.Storage.getUiSettings().assistantOpen===true;LF.Theme.apply(LF.Storage.getUiSettings().theme,false);
