(function () {
  ...
  function reportPreview(project) {
    ...
    const findingsBody = sections.has("ai") ? `<div class="report-findings-grid">${model.findings.map((finding) => `<article><div><strong>${finding.score}</strong><span>${esc(finding.status)}</span></div><h3>${esc(finding.title)}</h3><p>${esc(finding.detail)}</p>${report.includeEvidence ? `<small>${esc(finding.evidence)} · Simulated AI</small>` : ""}</article>`).join("")}</div>` : excluded("Evidence-Linked Findings");
