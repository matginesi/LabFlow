(function () {
  ...
  async function exportLabFlowZip(){if(!hasExperiment())return;flushDrafts();const exp=S.state.experiment;LF.UI.activityStart({title:'Export LabFlow ZIP',kind:'ZIP',stage:'Building portable save',progress:.15,cancellable:false,details:{Experiment:exp.meta.name,Revision:exp.sync&&exp.sync.revision||0}});try{const blob=await LF.Export.save(exp);C.downloadBlob(blob,LF.Export.fileName(exp));LF.UI.activityFinish({message:'Portable LabFlow ZIP created. The source archive inside it is unchanged.',details:{Bytes:blob.size,Revision:exp.sync&&exp.sync.revision||0},holdMs:0});LF.UI.message('LabFlow ZIP exported.','success');}catch(err){Log.error('export.labflow-failed',{error:err});LF.UI.activityError(err);LF.UI.message(err.message||String(err),'error');}}
  ...
  function bindEvents(){
    ...
    document.addEventListener('click',async function(e){
      try {
        ...
        const exportRepair=e.target.closest('[data-export-repair]');if(exportRepair){const exp=S.state.experiment,id=exportRepair.dataset.exportRepair;if(id==='complete-provenance'&&LF.NomadExport&&LF.NomadExport.repairPatchProvenance){const count=LF.NomadExport.repairPatchProvenance(exp);markModified('metadata');render();LF.UI.message(count?('Reconstructed provenance for '+count+' patch record'+(count===1?'':'s')+'.'):'No provenance repair was needed.',count?'success':'info');return;}if(id==='restore-source-name'&&exp&&exp.raw&&exp.raw.sourceName){exp.meta=exp.meta||{};exp.meta.sourceName=exp.raw.sourceName;markModified('metadata');render();LF.UI.message('Source archive metadata restored from the preserved RAW provenance.','success');return;}LF.UI.message('This export issue needs researcher review.','warning');return;}
        ...
        const applyReview=e.target.closest('[data-apply-review-proposal]');if(applyReview){
          ...
          if(p){try{const out=LF.DatasetCorrections.commitProposals(S.state.experiment,p,'ai',{actionId:'dataset.resolve-ambiguities',reason:'review-correction'});render();LF.UI.message('AI correction committed to the LabFlow Data ('+out.changed+' target'+(out.changed===1?'':'s')+', revision '+out.revisionAfter+').','success');}catch(err){p.applyError=err.message||String(err);Log.warn('review.proposal-apply-failed',{index:idx,error:err});render();LF.UI.message(p.applyError,'error');}}return;
        }
        if(e.target.closest('#applyAllAiCorrections')){
          ...
          try{const out=LF.DatasetCorrections.commitProposals(S.state.experiment,pending,'ai',{actionId:'dataset.resolve-ambiguities',reason:'review-corrections'});render();LF.UI.message((out.requested-out.failed)+' AI proposal'+((out.requested-out.failed)===1?'':'s')+' committed to '+out.changed+' target'+(out.changed===1?'':'s')+(out.failed?' · '+out.failed+' could not be mapped':''),out.failed?'warning':'success');}catch(err){render();LF.UI.message(err.message||String(err),'error');}return;
        }
        ...
        if(e.target.closest('#revalidateDataset')){refreshPipeline(S.state.experiment,'manual-refresh');if(LF.ActionData)LF.ActionData.removeProposal(S.state.experiment,'dataset.resolve-ambiguities');render();LF.UI.message('Canonical analysis rebuilt from the current LabFlow Data.','success');return;}
        
        if(e.target.closest('#refreshDesignEvidence')){if(LF.DesignModel&&LF.DesignModel.projectSource)LF.DesignModel.projectSource(S.state.experiment,true);refreshPipeline(S.state.experiment,'design-source-refresh');render();LF.UI.message('Design evidence re-read from the original source metadata.','success');return;}
        ...
        if(e.target.closest('#saveUserProfile')){LF.Storage.saveUserProfile({name:document.getElementById('userName').value.trim()||'Matteo Ginesi',organization:document.getElementById('userOrganization').value.trim(),email:document.getElementById('userEmail').value.trim()});LF.UI.message('Profile saved.','success');render();return;}
        ...
        if(e.target.closest('#reanalyzeDataset')){refreshPipeline(S.state.experiment,'manual-review');render();LF.UI.message('Deterministic data pipeline refreshed.','success');return;}
    ...
    document.addEventListener('submit',async function(e){
      if(e.target.id==='samplePatchForm'){
        ...
        try {
          ...
          const out=LF.DatasetCorrections.commitProposals(S.state.experiment,proposals,'user',{actionId:'review.manual-sample-mapping',reason:'review-correction'});render();LF.UI.message('Canonical sample mapping committed to '+out.changed+' measurement'+(out.changed===1?'':'s')+' at revision '+out.revisionAfter+'.','success');
  ...
  async function init(){
    ...
    try{
      ...
      if(saved&&saved.experiment&&saved.experiment.id){S.setExperiment(saved.experiment,saved.experiment.raw&&saved.experiment.raw.sourceArchive);restoreSavedUi(saved);try{refreshPipeline(S.state.experiment,'workspace-restore');}catch(err){Log.warn('workspace.restore-pipeline-failed',{experimentId:S.state.experiment.id,error:err});}Log.info('workspace.restored',{route:S.state.ui.route,experimentId:S.state.experiment.id,savedAt:saved.savedAt||''});}
      else{const aiSettings=LF.Storage.getAiSettings();S.resetSession();Log.info('workspace.empty-session',{route:S.state.ui.route,persistentProvider:true,persistentApiKey:!!LF.Storage.getApiKey(aiSettings.provider)});}
