...
(function () {
  ...
  function canonicalizeLocalBindAddress(){
    try{
      ...
      const url=new URL(location.href);url.hostname='127.0.0.1';location.replace(url.toString());return true;
    }catch(_){return false;}
  }
  if(canonicalizeLocalBindAddress())return;
...
existing=id&&id!=='__new__'?kb.get(id):null;
    return{id:existing&&existing.origin==='custom'?existing.id:undefined,kind:document.getElementById('kbKind').value,
    status:document.getElementById('kbStatus').value,title:document.getElementById('kbTitle').value,
    aliases:document.getElementById('kbAliases').value,tags:document.getElementById('kbTags').value,
    summary:document.getElementById('kbSummary').value,facts:document.getElementById('kbFacts').value,
    cautions:document.getElementById('kbCautions').value,related_ids:document.getElementById('kbRelatedIds').value,
    sources:kb.parseSourceLines(document.getElementById('kbSources').value),created_at:existing&&existing.created_at};}
  async function persistWorkspace(reason){if(!hasExperiment()||!LF.Storage||!LF.Storage.saveExperiment)return false;
if(workspaceSaveBusy){workspaceSavePending=true;return false;}workspaceSaveBusy=true;
    try{await LF.Storage.saveExperiment(S.state.experiment,workspaceUiSnapshot());
    Log.debug('workspace.autosaved',{reason:reason||'state',
    revision:S.state.experiment.sync&&S.state.experiment.sync.revision||0});return true;
    }catch(err){Log.warn('workspace.autosave-failed',{reason:reason||'state',error:err});return false;
    }finally{workspaceSaveBusy=false;if(workspaceSavePending){workspaceSavePending=false;scheduleWorkspaceSave('pending');
    }}}
  ...
  function restoreScrollableState(root,context){
    if(!root)return;const apply=function(){scrollMemoryNodes(root).forEach(function(el){
const state=scrollMemory.get(scrollNodeKey(el,root,context));if(!state)return;
      if(el.scrollTop!==undefined)el.scrollTop=Math.min(state.top,Math.max(0,el.scrollHeight-el.clientHeight));
      if(el.scrollLeft!==undefined)el.scrollLeft=Math.min(state.left,Math.max(0,el.scrollWidth-el.clientWidth));});};apply();
  ...
  function formatBrowserBytes(value){
    ...
    if(bytes>=1024*1024*1024)return (bytes/(1024*1024*1024)).toFixed(bytes>=10*1024*1024*1024?1:2)+' GB';
    if(bytes>=1024*1024)return (bytes/(1024*1024)).toFixed(bytes>=100*1024*1024?0:1)+' MB';
    if(bytes>=1024)return (bytes/1024).toFixed(1)+' KB';
    return Math.round(bytes)+' B';
  }

  function formatBrowserEta(value){
  ...
  function updateBrowserLocalSetupTotem(local){
  ...
  async function startBrowserLocal(forceDownload){
    ...
    let unsubscribe=null,setupTotem=false;
    try{
      const checked=await LF.BrowserLocal.check(settings.model);
      renderModelStatus();
      if(!checked.cached){
        setupTotem=true;
        if(LF.UI&&LF.UI.activityStart){
          LF.UI.activityStart({
            ...
            progress:.02,progressLabel:'Local model setup',indeterminate:false,cancellable:false,showAiTrace:false,
            details:{Model:checked.modelName||'LFM2.5 350M · Q4_K_M','Model size':formatBrowserBytes(LF.BrowserLocal.defaultModel.expectedBytes),Cache:'Not installed'},
            steps:browserLocalSetupSteps(checked)
          });
          updateBrowserLocalSetupTotem(checked);
        }
        unsubscribe=LF.BrowserLocal.subscribe(function(local){renderModelStatus();updateBrowserLocalSetupTotem(local);});
      }
      ...
      if(!ready.cached){
        ...
        if(setupTotem&&LF.UI&&LF.UI.activityError){
      ...
      if(setupTotem&&LF.UI&&LF.UI.activityFinish){
  ...
  function pageFailureHtml(route,error){
    ...
    return '<section class="page page-error-recovery"><div class="notice danger"><strong>'+C.escapeHtml(title)+
' could not be rendered.</strong><span>'+message+
      '</span></div><section class="panel"><div class="panel-head"><div><span class="eyebrow">Recovery</span><h2 ' +
        'class="h2">Fix this page here</h2><div class="meta">LabFlow will not send you back to Upload unless the source ' +
        'ZIP itself is missing.</div></div></div><div class="panel-body"><div class="row-wrap"><button type="button" class="button primary" data-retry-page>Retry page</button>'+
(canRebuild?'<button type="button" class="button" data-rebuild-derived>Rebuild derived data</button>':'')+
          '<button type="button" class="button" data-open-diagnostics>Open diagnostics</button></div></div></section></section>';
  }
...
toggle=document.getElementById('assistantToggle');
      ...
      if(assistant)assistant.hidden=!S.state.ui.assistantOpen;
      if(toggle){const assistantLabel=S.state.ui.assistantOpen?'Hide assistant':'Assistant';
      toggle.setAttribute('aria-pressed',S.state.ui.assistantOpen?'true':'false');
      toggle.innerHTML=(LF.Icons?LF.Icons.icon('message-square'):'')+'<span>'+assistantLabel+'</span>';}
    let html='';try{if(S.state.ui.route==='experiment-import')html=LF.ImportPage.render(S.state);
else if(S.state.ui.route==='experiment-results')html=LF.ResultsPage.render(S.state);
      else if(S.state.ui.route==='experiment-design')html=renderDesign();
      else if(S.state.ui.route==='cabinet')html=LF.CabinetPage.render();
      else if(S.state.ui.route==='experiment-export')html=LF.ExportPage.render(S.state);
      else if(S.state.ui.route==='documentation')html=LF.DocsPage.render();else html=LF.SettingsPage.render();
      }catch(err){Log.error('render.page-failed',{route:S.state.ui.route,error:err});
      html=pageFailureHtml(S.state.ui.route,err);}
    main.innerHTML=html;
...
S.state.ui.resultsOverviewMetric='eff';S.state.ui.resultsOverviewDirection='best';
        S.state.ui.resultsOverviewStatistic='median';S.state.ui.curveSelection=exp.measurements[0]?[exp.measurements[0].id]:[];
        S.state.ui.curveOverlaySelection=[];S.state.ui.curveView='all';S.state.ui.curveGroup='all';
        S.state.ui.curveDirection='both';S.state.ui.curveEligibleOnly=false;S.state.ui.curveSearch='';S.state.ui.curveZoom=1;
  ...
  async function exportLabFlowZip(){if(!hasExperiment())return;flushDrafts();const exp=S.state.experiment;
LF.UI.activityStart({title:'Export LabFlow ZIP',kind:'ZIP',stage:'Building portable save',progress:.15,
    cancellable:false,details:{Experiment:exp.meta.name}});try{const blob=await LF.Export.save(exp);
    C.downloadBlob(blob,LF.Export.fileName(exp));
    LF.UI.activityFinish({message:'Portable LabFlow ZIP created. The source archive inside it is unchanged.',details:{
...
const proposal=selectedDesignProposal(),list=proposal&&(kind==='solution'?proposal.solutions:proposal.devices),
    ...
    proposal.updatedAt=new Date().toISOString();markModified('ai');
    const main=document.getElementById('main'),top=main?main.scrollTop:0;render();
    requestAnimationFrame(function(){if(main)main.scrollTop=top;});}
...
const mobile=!!(window.matchMedia&&window.matchMedia('(max-width:1100px)').matches);
    ...
    if(toggle){toggle.setAttribute('aria-expanded',mobile&&next?'true':'false');
    toggle.setAttribute('aria-label',mobile&&next?'Close navigation':'Open navigation');
    }const sidebar=document.getElementById('primarySidebar');
    if(sidebar){if(mobile)sidebar.setAttribute('aria-hidden',next?'false':'true');
  ...
  function bindEvents(){
    ...
    document.addEventListener('click',async function(e){
      try {
        ...
        if(route){e.preventDefault();if(route.matches&&route.matches(':disabled')||
route.getAttribute('aria-disabled')==='true')return;closeMobileNav();
          if(S.state.ui.route==='experiment-design')commitDraft('design');const target=route.dataset.route;
          if(S.routeRequiresExperiment&&S.routeRequiresExperiment(target)&&!hasExperiment()){S.setRoute('experiment-import');
          LF.UI.message('Upload the original ZIP to open this workflow step.','info');return;}S.setRoute(target);return;}
        ...
        const seriesToggle=e.target.closest('[data-chart-series-toggle]');
if(seriesToggle){const wrap=seriesToggle.closest('.svg-chart-wrap')||seriesToggle.closest('.panel')||document,
          series=seriesToggle.dataset.chartSeriesToggle,next=!seriesToggle.classList.contains('active');
          seriesToggle.classList.toggle('active',next);seriesToggle.setAttribute('aria-pressed',String(next));
          wrap.querySelectorAll('[data-chart-series="'+series+'"]') .forEach(function(node){
          node.classList.toggle('chart-series-hidden',!next);});return;}
        const seriesAction=e.target.closest('[data-chart-series-action]');
if(seriesAction){const wrap=seriesAction.closest('.panel')||document,
          show=seriesAction.dataset.chartSeriesAction!=='hide-all';
          wrap.querySelectorAll('[data-chart-series-toggle]').forEach(function(button){button.classList.toggle('active',show);
          button.setAttribute('aria-pressed',String(show));});
          wrap.querySelectorAll('[data-chart-series]').forEach(function(node){node.classList.toggle('chart-series-hidden',!show);
          });return;}
        const chartExport=e.target.closest('[data-chart-export]');
if(chartExport){const format=chartExport.dataset.chartExport,id=chartExport.dataset.chartId,
          base=C.safeName(S.state.experiment&&S.state.experiment.meta&&S.state.experiment.meta.name||
          'labflow')+'_'+C.safeName(id||'chart');if(format==='svg')LF.ResultsPage.exportSvg(id,base+'.svg');
          else LF.ResultsPage.exportCanvas(id,base+'.png');return;}
...
if(!hasExperiment())throw new Error('No experiment is loaded.');
          refreshPipeline(S.state.experiment,'page-recovery:'+S.state.ui.route);render();
          LF.UI.message('Derived data rebuilt from the current LabFlow Data.','success');
          }catch(err){Log.error('render.recovery-failed',{route:S.state.ui.route,error:err});
          LF.UI.message(err.message||String(err),'error');}return;}
...
if(exportOption){const settings=Object.assign({}
          ...
          value=exportOption.dataset.exportOptionValue==='true';settings[key]=value;LF.Storage.saveExportSettings(settings);
          if(S.state.experiment&&S.state.experiment.nomad)S.state.experiment.nomad.mappingPlan=null;render();
          LF.UI.message('NOMAD package option updated.','success');return;}
        const exportFocus=e.target.closest('[data-export-focus]');
if(exportFocus){S.state.ui.exportFocus=exportFocus.dataset.exportFocus||'';render();
          setTimeout(function(){const target=S.state.ui.exportFocus==='patch-provenance'?
          document.getElementById('exportPatchProvenance'):document.getElementById('exportDangerFindings');
          if(target&&target.scrollIntoView)target.scrollIntoView({behavior:'smooth',block:'center'});},0);return;}
        const exportRepair=e.target.closest('[data-export-repair]');
if(exportRepair){const exp=S.state.experiment,id=exportRepair.dataset.exportRepair;
          if(id==='complete-provenance'&&LF.NomadExport&&LF.NomadExport.repairPatchProvenance){
          const count=LF.NomadExport.repairPatchProvenance(exp);markModified('metadata');render();
          LF.UI.message(count?('Completed change details for '+count+' '+(count===1?'record':'records')+
          '.'):'No change details needed repair.',count?'success':'info');return;
          }if(id==='restore-source-name'&&exp&&exp.raw&&exp.raw.sourceName){exp.meta=exp.meta||{};
          exp.meta.sourceName=exp.raw.sourceName;markModified('metadata');render();
          LF.UI.message('Source archive details restored from the original ZIP.','success');return;
          }LF.UI.message('This export issue needs researcher review.','warning');return;}
        if(e.target.closest('[data-export-refresh]')){
const exp=S.currentExperiment?S.currentExperiment('nomad-recheck'):S.state.experiment;
          S.commitAllDrafts&&S.commitAllDrafts();LF.UI.activityStart({
          title:'Recheck NOMAD readiness',subtitle:'Deterministic local validation',kind:'LOCAL',stage:'Refreshing LabFlow Data',
          progress:.25,cancellable:false,response:
          'Rebuilding the current mapping and validation from the same ExperimentData used by Results, Design, Actions and Assistant.'}
          );try{if(exp&&exp.nomad){exp.nomad.mappingPlan=null;exp.nomad.validation=null;
          }if(LF.DataPipeline&&LF.DataPipeline.refresh)LF.DataPipeline.refresh(exp,{reason:'nomad-recheck'});
          LF.UI.activityUpdate({stage:'Validating package',progress:.72});
          const validation=LF.NomadExport.validate(exp,exp.raw&&exp.raw.sourceArchive);render();
          LF.UI.activityFinish({message:validation.status==='blocked'?'NOMAD still needs attention.':'NOMAD readiness updated.',
          response:(validation.problems||[]).length?(validation.problems||[]).map(function(x){
          return (x.severity==='blocking'?'BLOCK':'REVIEW')+' · '+x.message;
          }).join('\n'):'Local staging checks passed.',details:{
          Status:validation.status,'Blocking issues':(validation.problems||[]).filter(function(x){return x.severity==='blocking';
          }).length,'Review items':(validation.problems||[]).filter(function(x){return x.severity!=='blocking';
          }).length},holdMs:0});}catch(error){LF.UI.activityError(error,{response:error.message||String(error),holdMs:0});}return;
          }
        ...
        const quick=e.target.closest('[data-results-quick]');if(quick){const q=quick.dataset.resultsQuick;
if(q==='jv'){S.state.ui.resultsJvMode='single';S.state.ui.curveView='single';S.state.ui.resultsTab='curves';
          }else if(q==='compare'){S.state.ui.resultsTab='boxplots';}else if(q==='warnings'){S.state.ui.resultsDataMode='warnings';
          S.state.ui.resultsTab='all';}else if(q==='top'){S.state.ui.resultsDataMode='top';S.state.ui.resultsTab='all';
          }else{S.state.ui.resultsDataMode='all';S.state.ui.resultsTab='all';}renderWithStableAnchor('.results-main-tabs');return;
          }
        ...
        const applyReview=e.target.closest('[data-apply-review-proposal]');if(applyReview){
          ...
          if(p){try{const out=LF.DatasetCorrections.commitProposals(S.state.experiment,p,'ai',{
actionId:'dataset.resolve-ambiguities',reason:'review-correction'});render();
            LF.UI.message('AI suggestion applied to '+out.changed+' '+(out.changed===1?'record':'records')+'.','success');
            }catch(err){p.applyError=err.message||String(err);Log.warn('review.proposal-apply-failed',{index:idx,error:err});
            render();LF.UI.message(p.applyError,'error');}}return;
        }
...
actionId:'dataset.resolve-ambiguities',reason:'review-corrections'});render();
            LF.UI.message('Applied '+(out.requested-out.failed)+' AI suggestion'+((out.requested-out.failed)===1?'':'s')+' to '+
            out.changed+' '+(out.changed===1?'record':'records')+(out.failed?' · '+out.failed+' could not be applied':''),
            out.failed?'warning':'success');}catch(err){render();LF.UI.message(err.message||String(err),'error');}return;
        }
        ...
        if(e.target.closest('#applyAutomaticCleanup')){
          ...
          try{const out=LF.DatasetCorrections.commitAutomaticSafeFixes(exp);render();
LF.UI.message('Applied '+Number(out.lastApplied||0)+' safe correction'+(Number(out.lastApplied||
            0)===1?'':'s')+' to '+Number(out.targets||0)+' '+(Number(out.targets||0)===1?'record':'records')+'.','success');
            }catch(err){Log.warn('review.automatic-cleanup-apply-failed',{error:err});
            refreshPipeline(exp,'automatic-cleanup-recovery');render();LF.UI.message(err.message||String(err),'error');}return;
        }
        ...
        if(e.target.closest('#applyAllReviewFixes')){
          ...
          try{const out=LF.DatasetCorrections.commitProposals(S.state.experiment,items.map(function(fix){
return Object.assign({},fix);}), 'user',{actionId:'review.exclude-measurements',reason:'review-exclusions'});render();
            LF.UI.message((out.requested-out.failed)+' exclusion'+((out.requested-out.failed)===1?'':'s')+' committed'+
            (out.failed?' · '+out.failed+' failed':''),out.failed?'warning':'success');}catch(err){render();
            LF.UI.message(err.message||String(err),'error');}return;
        }
        const localFix=e.target.closest('[data-local-fix]');if(localFix){
          const m=S.state.experiment.measurements.find(function(x){return x.id===localFix.dataset.measurementId;});
if(m){const exclude=localFix.dataset.localFix==='exclude',proposal={
            patch_type:exclude?'exclude_measurement':'restore_measurement',target:m.id,before:!!m.excluded,after:exclude,
            reason:'Review data local correction',evidence:['Researcher-confirmed local correction']};
            try{const out=LF.DatasetCorrections.commitProposals(S.state.experiment,proposal,'user',{
            actionId:'review.measurement-eligibility',reason:'review-correction'});render();
            LF.UI.message(exclude?'Measurement excluded from analysis.':'Measurement restored to analysis.','success');
        ...
        if(e.target.closest('#applyAllDesignSuggestions')){try{
const out=LF.DesignAnalysis.applyAll(S.state.experiment,S.state.ui.selectedDesignDeviceId);
          if(out.changed){markModified('design');render();
          LF.UI.message('Applied '+out.changed+' AI-proposed missing field'+(out.changed===1?'':'s')+' to Design.','success');
          }else{render();LF.UI.message('No missing field could be filled without overwriting existing values.','info');
          }}catch(err){LF.UI.message(err.message||String(err),'error');}return;}
        if(e.target.closest('#acceptAllDesignInferences')){
const exp=S.state.experiment,count=Object.keys(LF.ActionData?LF.ActionData.proposals(exp,'design.infer'):{}).length;
          if(!count){LF.UI.message('No AI Design suggestions are waiting for acceptance.','info');return;
          }const ok=await LF.UI.confirmAction('Accept all '+count+' current AI Design suggestion'+(count===1?'':'s')+
        ...
        const applyDesignProposal=e.target.closest('[data-apply-design-proposal]');
if(applyDesignProposal){try{const out=LF.DesignAnalysis.applyOne(S.state.experiment,
          applyDesignProposal.dataset.applyDesignProposal,Number(applyDesignProposal.dataset.proposalIndex),
          applyDesignProposal.dataset.proposalPart||'all',S.state.ui.selectedDesignDeviceId);markModified('design');render();
          LF.UI.message('AI suggestion applied to '+out.changed+' missing field'+(out.changed===1?'':'s')+'.','success');
          }catch(err){LF.UI.message(err.message||String(err),'error');}return;}
        const applyDesignDevice=e.target.closest('[data-apply-design-device]');
if(applyDesignDevice){try{const out=LF.DesignAnalysis.applySelectedDevice(S.state.experiment,
          applyDesignDevice.dataset.applyDesignDevice,S.state.ui.selectedDesignDeviceId);markModified('design');render();
          LF.UI.message('Applied AI-suggested missing values to '+out.changed+' Design field(s) for the selected experiment.',
...
if(discardDesignExperiment){const exp=S.state.experiment,
          id=String(discardDesignExperiment.dataset.discardDesignExperiment||'');
          if(LF.ActionData){LF.ActionData.removeProposal(exp,'design.infer',id);
          LF.ActionData.setStatus(exp,'design.infer',id,{state:'idle',updatedAt:new Date().toISOString(),message:''});
          }markModified('ai');render();LF.UI.message('AI suggestion discarded.','info');return;}
        ...
        const row=e.target.closest('[data-measurement-row]');
if(row){S.state.ui.selectedMeasurementId=row.dataset.measurementRow;
          const inlineDesktop=!!row.closest('.results-master-detail')&&window.matchMedia&&
          window.matchMedia('(min-width: 1181px)').matches;if(inlineDesktop){S.state.ui.resultInspectorId=null;render();
          }else{S.state.ui.resultInspectorId=row.dataset.measurementRow;LF.ResultsPage.renderResultInspector();}return;}
    ...
    document.addEventListener('change',function(e){
      try {
        ...
        const curve=e.target.closest('[data-curve-check]');
if(curve){S.state.ui.curveOverlaySelection=S.state.ui.curveOverlaySelection||[];
          if(curve.checked&&!S.state.ui.curveOverlaySelection.includes(curve.dataset.curveCheck))S.state.ui.curveOverlaySelection.push(curve.dataset.curveCheck);
          if(!curve.checked)S.state.ui.curveOverlaySelection=S.state.ui.curveOverlaySelection.filter(function(id){
          return id!==curve.dataset.curveCheck;});render();return;}
    ...
    document.addEventListener('input',function(e){
      try {
        ...
        const proposalSolutionField=e.target.closest('[data-proposal-solution-index]');
if(proposalSolutionField){const proposal=selectedDesignProposal(),
          item=proposal&&proposal.solutions&&proposal.solutions[Number(proposalSolutionField.dataset.proposalSolutionIndex)];
          if(item){const field=proposalSolutionField.dataset.proposalField,previous=item[field];
          item[field]=proposalSolutionField.value;
          if(field==='name'&&String(previous)!==item.name)(proposal.devices||[]).forEach(function(device){
          device.solution_names=(device.solution_names||[]).map(function(name){
          return String(name)===String(previous)?item.name:name;});});if(item.applied){item.applied=false;item.decision='pending';
          }proposal.userEdited=true;proposal.updatedAt=new Date().toISOString();markModified('ai');}return;}
        const proposalLayerField=e.target.closest('[data-proposal-layer-field]');
if(proposalLayerField){const proposal=selectedDesignProposal(),
          device=proposal&&proposal.devices&&proposal.devices[Number(proposalLayerField.dataset.proposalDeviceIndex)],
          layer=device&&device.stack&&device.stack[Number(proposalLayerField.dataset.proposalLayerIndex)];
          if(layer){layer[proposalLayerField.dataset.proposalLayerField]=proposalLayerField.value;
          if(device.applied){device.applied=false;device.decision='pending';}proposal.userEdited=true;
          proposal.updatedAt=new Date().toISOString();markModified('ai');}return;}
        const proposalProcessField=e.target.closest('[data-proposal-process-field]');
if(proposalProcessField){const proposal=selectedDesignProposal(),
          item=proposal&&proposal.devices&&proposal.devices[Number(proposalProcessField.dataset.proposalDeviceIndex)];
          if(item){item.process=item.process||{};item.process[proposalProcessField.dataset.proposalProcessField]=proposalProcessField.value;
          if(item.applied){item.applied=false;item.decision='pending';}proposal.userEdited=true;
          proposal.updatedAt=new Date().toISOString();markModified('ai');}return;}
        const proposalDeviceField=e.target.closest('[data-proposal-device-index][data-proposal-field]');
if(proposalDeviceField){const proposal=selectedDesignProposal(),
          item=proposal&&proposal.devices&&proposal.devices[Number(proposalDeviceField.dataset.proposalDeviceIndex)];
          if(item){const value=proposalDeviceField.dataset.proposalArray==='true'?
          proposalDeviceField.value.split(',').map(function(x){return x.trim();}).filter(Boolean):proposalDeviceField.value;
          item[proposalDeviceField.dataset.proposalField]=value;if(item.applied){item.applied=false;item.decision='pending';
          }proposal.userEdited=true;proposal.updatedAt=new Date().toISOString();markModified('ai');}return;}
