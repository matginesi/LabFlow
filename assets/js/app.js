  ...
  function bindEvents(){
    ...
    document.addEventListener('click',async function(e){
      try {
        ...
        const curveSelect=e.target.closest('[data-curve-select]');if(curveSelect){e.preventDefault();S.state.selectedMeasurementId=curveSelect.dataset.curveSelect;S.state.curveSelection=[curveSelect.dataset.curveSelect];renderKeepingAnchor('.jv-analyzer-layout');return;}
    ...
    document.addEventListener('input',function(e){
      try {
        ...
        if(e.target.id==='kbSettingsSearch'){S.state.ui=S.state.ui||{};S.state.ui.settingsKnowledgeQuery=e.target.value;clearTimeout(S.state._kbSettingsSearchTimer);S.state._kbSettingsSearchTimer=setTimeout(function(){render();const input=document.getElementById('kbSettingsSearch');if(input){input.focus();input.setSelectionRange(input.value.length,input.value.length);}},150);return;}
