...
register({id:'auto-cleanup',phase:'review',after:['review'],reads:['datasetAnalysis'],writes:['autoCleanup','patches'],run:function(exp){if(!LF.DatasetCorrections||!LF.DatasetCorrections.prepareAutomaticSafeFixes)return null;const out=LF.DatasetCorrections.prepareAutomaticSafeFixes(exp);if(out&&Number(out.withdrawn||0)>0){if(LF.DataModel&&LF.DataModel.touch)LF.DataModel.touch(exp,'dataset');return{restartFrom:'link'};}return null;},description:'Detect mechanically provable cleanup automatically, keep it pending for explicit researcher acceptance, and withdraw unsafe legacy automatic exclusions.'});
