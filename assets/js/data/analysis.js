...
experimentId:m.experimentId||'', group:m.group, position:m.position||'', cell:m.cell||'', runId:m.runId||null,
      sequence:m.sequence==null?null:m.sequence, isRef:m.isRef, bestEff:m.bestEff, hysteresis:m.hysteresis,
      qualityStatus:m.qualityStatus, flags:(m.flags||[]).map(function(f){return f.label;}) };
  }
  ...
  function toCSV(exp,opts) {
    ...
    const rows = measurements.map(function (m) {
      return [m.file,m.experiment||m.group||'',m.sample,m.position||'',m.cell||'',m.runId||'',
m.sequence==null?'':m.sequence,m.group,m.isRef,m.qualityStatus,!!m.excluded,m.rankingEligible,(m.fw||{}).voc,(m.fw||{}
        ).jsc,(m.fw||{}).vmpp,(m.fw||{}).jmpp,(m.fw||{}).pmpp,(m.fw||{}).rs,(m.fw||{}).rsh,(m.fw||{}
        ).ff,Number.isFinite((m.fw||{}).eff)?(m.fw||{}).eff/factor:'',(m.rv||{}).voc,(m.rv||{}).jsc,(m.rv||{}).vmpp,(m.rv||{}
        ).jmpp,(m.rv||{}).pmpp,(m.rv||{}).rs,(m.rv||{}).rsh,(m.rv||{}).ff,Number.isFinite((m.rv||{}).eff)?(m.rv||{}
        ).eff/factor:'',m.hysteresis,factor,(m.flags||[]).map(function(f){return f.label;
        }).join('; ')].map(C.csvEscape).join(',');
    });
    return h.join(',') + '\n' + rows.join('\n');
  }
