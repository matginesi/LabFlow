(function () {
  ...
  function editorStage(type, ctx) {
    ...
    return `<section class="tool-stage"><div class="tool-stage-head"><div><span class="page-eyebrow">${labels[type][0]}</span><h2>${labels[type][1]}</h2></div><div class="cluster"><label class="btn btn-sm" for="tool-file-input">${icon("upload")} Open local file</label><input class="sr-only" id="tool-file-input" type="file" accept=".${type === "markdown" ? "md" : type === "latex" ? "tex" : type},text/plain"><button class="btn btn-primary btn-sm" id="tool-download">${icon("download")} Download</button></div></div>
  ...
  function spreadsheetStage(ctx) {
    ...
    return `<section class="tool-stage"><div class="tool-stage-head"><div><span class="page-eyebrow">Workbook</span><h2>Spreadsheet with sheets</h2></div><div class="cluster"><button class="btn btn-sm" id="sheet-add">${icon("plus")} Add sheet</button><button class="btn btn-primary btn-sm" id="xlsx-download">${icon("download")} Download XLSX</button></div></div>
  ...
  function diagramStage(ctx) {
    ...
    return `<section class="tool-stage"><div class="tool-stage-head"><div><span class="page-eyebrow">Diagram Studio</span><h2>Research workflows and evidence graphs</h2></div><div class="cluster"><label class="sr-only" for="diagram-template">Diagram template</label><select class="select select-sm" id="diagram-template"><option value="">Choose template…</option>${templateOptions}</select><button class="btn btn-sm" id="diagram-validate">${icon("check")} Validate</button><button class="btn btn-primary btn-sm" id="diagram-download">${icon("download")} Download SVG</button></div></div>
      <div class="tool-document-bar diagram-document-bar"><input class="input" id="diagram-filename" value="${esc(diagramState.name)}" aria-label="Diagram file name"><div class="segmented" aria-label="Diagram direction"><button type="button" data-diagram-direction="TD">Top to bottom</button><button type="button" data-diagram-direction="LR">Left to right</button></div><span class="toolbar-spacer"></span><span class="diagram-stat" id="diagram-node-count">0 nodes</span><span class="diagram-stat" id="diagram-edge-count">0 relations</span></div>
