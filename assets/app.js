document.addEventListener('DOMContentLoaded', () => {
  ...
  const navigation = [
    ...
    { id: 'dashboard',   href: 'index.html',       icon: 'home',    label: 'Workspace' },
    { id: 'processes',   href: 'processes.html',   icon: 'flow',    label: 'Processes' },
    { id: 'experiments', href: 'experiments.html', icon: 'flask',   label: 'Experiments' },
    { id: 'catalogs',    href: 'catalogs.html',    icon: 'grid',    label: 'Lab Cabinet' },
    { id: 'imports',     href: 'imports.html',     icon: 'upload',  label: 'Imports' },
    { id: 'report',      href: 'report.html',      icon: 'report',  label: 'Reports' },
    { id: 'editors',     href: 'editors.html',     icon: 'code',    label: 'Tools' },
    { section: true, label: 'SPECIALIST' },
    { id: 'nomad',     href: 'report.html#nomad',        icon: 'download',label: 'NOMAD' },
    { id: 'knowledge', href: 'knowledge.html',           icon: 'model',   label: 'Knowledge Assistant' },
    { id: 'ai',        href: 'ai-assistant.html',        icon: 'spark',   label: 'AI Analysis & Agents' },
    ...
    { id: 'admin-settings', href: 'admin-settings.html', icon: 'shield',  label: 'Admin Settings' },
  ];
  ...
  function mountShell() {
    ...
    if (sidebar) {
      const navHtml = navigation.map(item => {
        ...
        return `<a class="nav-link ${navActive(item.id) ? 'active' : ''}" href="${item.href}">${icon(item.icon)}<span>${item.label}</span></a>`;
      }).join('');
      sidebar.innerHTML = `<nav class="sidebar-nav">${navHtml}</nav><div class="sidebar-foot"><strong>${escapeHtml(user.workspace)}</strong><br><span>${escapeHtml(process.shortName||process.name)}</span><br>${escapeHtml(user.privateLabel)} · ${escapeHtml(user.id)}<br>No CDN · no trackers</div>`;
    ...
    mountUserPopover(); mountProcessPopover(); mountScopeBar(); mountCreateWizard(); mountProjectWizard(); applyUserScope();
    }
  }
  ...
  function mountProcessPopover(){
    ...
    pop.innerHTML=`<div class="user-popover-head"><strong>Your processes</strong><small>A process defines the reusable workflow; experiments record its concrete execution.</small></div>${items}<div class="popover-actions"><a class="button small" href="processes.html">Manage processes</a><a class="button small" href="projects.html">Optional projects</a></div>`;
    document.body.append(pop);
  }

  function mountScopeBar(){
    if(page==='kit'||page==='dashboard') return;
    const header=$('.page-header'); if(!header||$('.scope-bar')) return; const u=currentUser(); const project=currentProject(); const process=currentProcess();
    const bar=document.createElement('section'); bar.className='scope-bar project-scope';
    bar.innerHTML=`<div class="scope-main">${icon('lock')}<div><span>Private research scope</span><strong>${escapeHtml(u.name)} <b>›</b> ${escapeHtml(u.workspace)} <b>›</b> ${escapeHtml(process.name)}</strong></div></div><div class="scope-meta"><span class="badge success">Owner only</span><span class="mono">${escapeHtml(process.id)}</span><span class="badge">Project optional · ${escapeHtml(project.id)}</span><button class="button small" type="button" data-action="process-toggle">Switch process</button></div>`;
  ...
  const createTypes={
    ...
    process:{label:'Process',copy:'Reusable workflow connecting resources, expected evidence and experiments',icon:'flow'},
    experiment:{label:'New experiment',copy:'Prepare solutions and stacks before starting laboratory work',icon:'flask'}
  };
  ...
  const globalSearchItems=[
    ['Experiment PSC-2026-041','Continue solutions, stacks or processing','experiment.html','experiment solution stack processing PSC'],
    ['Processes','Browse reusable laboratory workflows','processes.html','process workflow definition evidence'],
    ['Perovskite solar cell fabrication — n-i-p','Open the current process definition','pipeline.html','process workflow protocol resources NOMAD'],
    ['Experiments','Browse and continue laboratory experiments','experiments.html','experiment process project optional'],
    ['FA–Cs 1.2 M · SOL-081','Open the current solution and prepared batch','solution.html','solution solvent solute DMF DMSO'],
    ['Stacks S01 and S02','Inspect layers, conditions and samples','experiment.html#stacks','stack material layer sample'],
    ['Spin Coater 01','Open the shared equipment resource','catalogs.html','equipment instrument spin coating'],
    ['PSC n-i-p standard · v4.1','Open the approved reusable protocol in the Lab Cabinet','catalogs.html','protocol processing action cabinet'],
    ['Import data','Preview files, manual records or NOMAD entries','imports.html','file csv excel json import'],
    ['Results and comparisons','Search samples, measurements and results','workspace.html','data result measurement chart'],
    ['Report & Export Center','Edit the report and generate local files','report.html','report pdf excel csv export'],
    ['Prepare NOMAD','Validate, package or simulate API submission','report.html#nomad','nomad archive api export'],
    ['Laboratory tools','Open mapping, chart and conversion tools','editors.html','tools editor converter validation'],
    ['Knowledge Assistant','Find cited shared laboratory knowledge','knowledge.html','knowledge RAG source evidence'],
    ['AI Analysis','Analyse selected experimental data','ai-assistant.html#analysis','AI analysis anomaly'],
    ['AI Agents','Review guided multi-step proposals','ai-assistant.html#agents','AI agent approval workflow'],
    ['User Settings','Preferences and personal NOMAD connection','users.html','settings profile api key'],
    ['Admin Settings','Workspace policies and shared configuration','admin-settings.html','admin settings workspace'],
    ['User manual','Understand LabFlow concepts and workflow','documentation.html','documentation manual help'],
    ['UI Kit','Inspect canonical components and states','ui-kit.html','design component interface']
  ].map(([title,copy,href,keywords])=>({title,copy,href,keywords}));
  const searchPlaceholders={dashboard:'Search processes and experiments…',processes:'Search processes…',process:'Search process phases and evidence…',experiments:'Search experiments…',experiment:'Search this experiment…',catalogs:'Search Lab Cabinet…',docs:'Search documentation…',knowledge:'Search knowledge…',report:'Search reports & exports…',editors:'Search laboratory tools…',users:'Search settings…',workspace:'Search stacks, files & results…'};
  function searchPlaceholder(key){return searchPlaceholders[key]||'Search LabFlow…';}
  ...
  function mountGlobalSearch(){let results=$('#globalSearchResults');if(results)return;results=document.createElement('section');results.id='globalSearchResults';results.className='global-search-results';results.hidden=true;results.setAttribute('aria-live','polite');$('#topbar')?.append(results);}
  ...
  function mountProjectWizard(){
    ...
    wrap.innerHTML=`<section class="modal project-wizard-modal"><div class="panel-header"><div class="panel-title"><strong>Create a project</strong><small>Three clear decisions, then start the first experiment.</small></div><button class="icon-button" type="button" data-action="close" aria-label="Close">×</button></div><div id="projectWizardProgress" class="create-progress"></div><div class="panel-body" id="projectWizardBody"></div><div class="panel-footer" id="projectWizardFooter"></div></section>`;
  ...
  function renderProjectWizard(){
    ...
    if(projectWizardState.step===0)body.innerHTML=`<div class="starter-choice"><button class="starter-card active" type="button" data-project-starter="research"><span class="badge info">Optional container</span><strong>Group related experiments</strong><small>Use a project only when several experiments share one research goal.</small></button></div><div class="project-preview wizard-preview"><span class="project-preview-root">${icon('folder')}<strong>Project</strong></span><span class="project-preview-arrow">→</span><span>Experiments</span><span>Results</span><span>Report</span><span>NOMAD</span></div>`;
  ...
  function renderDashboard(){
    ...
    if(projects) projects.innerHTML=experimentRows.slice(0,3).map((row,index)=>`<article class="home-project-card experiment-home-card"><div class="home-project-card-head"><span class="home-project-folder">${icon('flask')}</span><div><span class="eyebrow mono">${row.id}</span><h3>${escapeHtml(row.name)}</h3></div><span class="badge ${row.status==='completed'?'success':'warning'}">${row.statusLabel}</span></div><p class="home-experiment-objective">${index===0?'Compare solvent systems and annealing temperatures.':index===1?'Refine the antisolvent application window.':'Compare absorber concentration and film response.'}</p><div class="home-project-summary"><span><strong>${index===0?2:index===1?3:2}</strong> stacks</span><span>Updated <strong>${row.date.toLowerCase()}</strong></span></div><div class="home-project-next"><span>Recommended next step</span><strong>${index===0?'Import measurement data for S01 and S02':index===1?'Complete the process for S03':'Compare the stack results'}</strong></div><div class="home-project-footer"><small>${escapeHtml(row.stack)}</small>${index===0?'<a class="button small primary" href="experiment.html#data">Continue</a>':row.actionLink}</div></article>`).join('');
    const attention=$('#homeAttentionList');
    if(attention){
      const recent=currentUser().recent;
      const project=currentProject();
      attention.innerHTML=[
        [recent,'Two stacks are ready for measurement import','Add data','experiment.html#data','warning'],
        [`${project.id} export`,'NOMAD package requires a final metadata review','Review','report.html#nomad','info'],
  ...
  const cabinetFavorites=new Set(cabinetResources.filter(item=>item.favorite).map(item=>item.id));
  let cabinetCategory='all',cabinetView='cards';
  function cabinetResourceCard(item){
    return `<article class="cabinet-resource-card" data-cabinet-item="${item.id}"><header><span class="cabinet-resource-mark ${item.category}">${escapeHtml(item.icon)}</span><div><span class="eyebrow">${escapeHtml(item.type)}</span><h3>${escapeHtml(item.name)}</h3></div><button class="cabinet-favorite ${cabinetFavorites.has(item.id)?'active':''}" type="button" data-action="cabinet-favorite" data-resource-id="${item.id}" aria-label="Toggle favorite">★</button></header><p>${escapeHtml(item.description)}</p><div class="cabinet-resource-meta"><span class="badge ${item.tone}">${escapeHtml(item.status)}</span><span>${icon('folder')}${escapeHtml(item.location)}</span><span>${item.uses} experiment uses</span></div><small>${escapeHtml(item.meta)}</small><footer><button class="button small primary" type="button" data-action="cabinet-use" data-resource-id="${item.id}">Use</button><button class="button small" type="button" data-action="cabinet-open" data-resource-id="${item.id}">Open</button><button class="icon-button small" type="button" data-action="cabinet-more" data-message="Duplicate · view uses · archive">•••</button></footer></article>`;
  }
  function cabinetFilteredResources(){
    const q=($('#cabinetSearch')?.value||'').trim().toLowerCase(),availability=$('#cabinetAvailability')?.value||'all',location=$('#cabinetLocation')?.value||'all',favorites=$('#cabinetFavoritesOnly')?.checked;
    return cabinetResources.filter(item=>(cabinetCategory==='all'||item.category===cabinetCategory)&&(!q||`${item.name} ${item.type} ${item.description} ${item.meta}`.toLowerCase().includes(q))&&(availability==='all'||item.availability===availability)&&(location==='all'||item.locationKey===location)&&(favorites?cabinetFavorites.has(item.id):true));
  }
  function renderCabinet(){
    const grid=$('#cabinetResourceGrid'),rows=$('#cabinetResourceRows');if(!grid&&!rows)return;
    const items=cabinetFilteredResources();
    if(grid)grid.innerHTML=items.map(cabinetResourceCard).join('');
    if(rows)rows.innerHTML=items.map(item=>`<tr><td><div class="cell-main"><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.description)}</small></div></td><td>${escapeHtml(item.type)}</td><td><span class="badge ${item.tone}">${escapeHtml(item.status)}</span></td><td>${escapeHtml(item.location)}</td><td>${item.uses} experiments</td><td><button class="button small" type="button" data-action="cabinet-open" data-resource-id="${item.id}">Open</button></td></tr>`).join('');
  ...
  function openCabinetDetail(id){
    ...
    body.innerHTML=`<section class="cabinet-detail-hero"><span class="cabinet-resource-mark ${item.category}">${escapeHtml(item.icon)}</span><div><span class="eyebrow">${escapeHtml(item.type)}</span><h2>${escapeHtml(item.name)}</h2><p>${escapeHtml(item.description)}</p></div><button class="cabinet-favorite ${cabinetFavorites.has(item.id)?'active':''}" type="button" data-action="cabinet-favorite" data-resource-id="${item.id}" aria-label="Toggle favorite">★</button></section><section class="cabinet-detail-status"><span class="badge ${item.tone}">${escapeHtml(item.status)}</span><span>${icon('folder')} ${escapeHtml(item.location)}</span><span>${item.uses} experiment uses</span></section><section class="cabinet-detail-section"><h3>Key information</h3><dl class="key-values">${item.details.map(([key,value])=>`<div><dt>${escapeHtml(key)}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl></section>${item.category==='solutions'?'<section class="cabinet-solution-visual"><div><i style="width:80%"></i><strong>DMF 80%</strong></div><div><i style="width:20%"></i><strong>DMSO 20%</strong></div><small>Expected appearance · clear amber · no precipitation</small></section>':''}${item.category==='equipment'?'<section class="cabinet-capabilities"><span>Supported actions</span><b>Spin coating</b><b>Dripping</b><b>Two-step coating</b></section>':''}<section class="cabinet-detail-section"><h3>Used in experiments</h3><article class="cabinet-usage-record"><span class="badge info">Snapshot retained</span><strong>${escapeHtml(item.usage)}</strong><small>Local parameters and actual values remain in the experiment.</small><a href="experiment.html#resources">Open experiment usage</a></article></section><footer class="cabinet-detail-actions"><button class="button primary" type="button" data-action="cabinet-use" data-resource-id="${item.id}">Use in experiment</button><button class="button" type="button" data-action="toast" data-message="An editable copy was created">Duplicate</button><button class="button" type="button" data-action="toast" data-message="Resource editor opened">Edit</button></footer>`;
    drawer.hidden=false;
  }
  ...
  const scientificTools={
    ...
    validate:[
      ['Experiment completeness','Experiment graph','Issue list','code','Check required fields, files, stack links and provenance.','Ready'],
      ...
      ['NOMAD compatibility','Experiment package','Mapping issues','code','Explain blockers before package generation.','Ready']
    ],
    nomad:[
      ['Mapping preview','Experiment graph','NOMAD field map','code','Compare local records with the planned NOMAD schema.','Ready'],
  ...
  function renderToolCatalog(category='data'){
    ...
    root.innerHTML=(scientificTools[category]||[]).map(([name,input,output,target,copy,status],index)=>`<article class="scientific-tool-card"><header><span class="tool-index">${String(index+1).padStart(2,'0')}</span><span class="badge ${status==='Ready'?'success':status==='Simulated'?'violet':'info'}">${status}</span></header><div><h3>${escapeHtml(name)}</h3><p>${escapeHtml(copy)}</p></div><dl><div><dt>Input</dt><dd>${escapeHtml(input)}</dd></div><div><dt>Output</dt><dd>${escapeHtml(output)}</dd></div></dl><footer><span>PSC-2026-041 · S02</span><button class="button small primary" type="button" data-action="tool-open-catalog" data-tool-target="${target}" data-tool-name="${escapeHtml(name)}">Open tool</button></footer></article>`).join('');
  }
  ...
  function mountCabinetSelector(){
    ...
    drawer.innerHTML=`<div class="panel-header"><div class="panel-title"><strong>Choose from Lab Cabinet</strong><small>For PSC-2026-041 · resources become traceable snapshots</small></div><button class="icon-button" type="button" data-action="close" aria-label="Close">×</button></div><div class="cabinet-selector-toolbar"><label class="cabinet-search"><span>${icon('model')}</span><input id="cabinetSelectorSearch" type="search" placeholder="Search shared resources…"></label><div class="cabinet-selector-context"><span class="badge info">Current process</span><strong>${escapeHtml(currentProcess().name)}</strong></div></div><div class="cabinet-selector-body" id="cabinetSelectorBody"></div><div class="cabinet-selector-footer"><button class="button" type="button" data-action="cabinet-context-create">${icon('plus')} Create missing resource</button><small>Nothing is removed from the shared record.</small></div>`;
    document.body.append(drawer);
  }
  function selectorItems(){
    const q=($('#cabinetSelectorSearch')?.value||'').trim().toLowerCase();
    return cabinetResources.filter(item=>(cabinetSelectorCategory==='all'||item.category===cabinetSelectorCategory)&&(!q||`${item.name} ${item.type} ${item.meta}`.toLowerCase().includes(q)));
  }
  function renderCabinetSelector(){
    const root=$('#cabinetSelectorBody');if(!root)return;const items=selectorItems();
    const row=item=>`<article class="cabinet-selector-row"><span class="cabinet-resource-mark ${item.category}">${escapeHtml(item.icon)}</span><div><span class="eyebrow">${escapeHtml(item.type)}</span><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.location)} · ${escapeHtml(item.meta)}</small></div><span class="badge ${item.tone}">${escapeHtml(item.status)}</span><button class="button small primary" type="button" data-action="cabinet-select-use" data-resource-id="${item.id}" ${item.availability==='unavailable'?'disabled title="Currently unavailable"':''}>${item.availability==='unavailable'?'Unavailable':'Use'}</button></article>`;
    const favorites=items.filter(item=>cabinetFavorites.has(item.id)),recent=items.filter(item=>item.recent&&!cabinetFavorites.has(item.id)),others=items.filter(item=>!item.recent&&!cabinetFavorites.has(item.id));
    root.innerHTML=items.length?`${favorites.length?`<div class="selector-group"><h3>Favorites</h3>${favorites.map(row).join('')}</div>`:''}${recent.length?`<div class="selector-group"><h3>Recently used</h3>${recent.map(row).join('')}</div>`:''}${others.length?`<div class="selector-group"><h3>Other matching resources</h3>${others.map(row).join('')}</div>`:''}`:`<div class="useful-empty"><span>${icon('model')}</span><div><strong>No matching resource</strong><small>Create the minimum without leaving the experiment.</small></div><button class="button primary" type="button" data-action="cabinet-context-create">Create resource</button></div>`;
  }
  function renderContextCreate(){
    const root=$('#cabinetSelectorBody');if(!root)return;
    root.innerHTML=`<section class="cabinet-context-create"><button class="button small" type="button" data-action="cabinet-selector-back">← Back to results</button><div><span class="eyebrow">Minimal record</span><h2>Create and use</h2><p>Add only what is needed now. You can complete identifiers and documentation later.</p></div><div class="field"><label>Type</label><select class="input" id="contextCreateType"><option>Material or substance</option><option>Solution or batch</option><option>Substrate</option><option>Instrument</option><option>Protocol or action</option><option>Condition</option><option>Stack template</option></select></div><div class="field"><label>Name</label><input class="input" id="contextCreateName" placeholder="Resource name"></div><div class="cabinet-duplicate-check" id="contextDuplicateCheck"><span>${icon('info')}</span><div><strong>Duplicate check ready</strong><small>Enter a name to compare it with shared records.</small></div></div><fieldset class="cabinet-visibility-choice"><legend>Visibility</legend><label><input checked name="contextVisibility" type="radio" value="shared"><span><strong>Share in Lab Cabinet</strong><small>Reusable by this workspace.</small></span></label><label><input name="contextVisibility" type="radio" value="local"><span><strong>Only this experiment</strong><small>Local snapshot, not shared.</small></span></label></fieldset><button class="button primary w-100" type="button" data-action="cabinet-context-confirm">Create and use in experiment</button></section>`;
  }
  ...
  const aiModes={
    plan:{title:'Plan an experiment',subtitle:'Use the active protocol and prior experiments as context.',badge:'Experiment copilot',prompts:['Suggest 2 simple conditions','Reduce the number of samples','Explain why this plan is useful'],context:['PSC n-i-p standard · v4.1','Current experiment PSC-2026-041','Allowed variable: spin speed','Target measurements: J–V, EQE, XRD'],response:{summary:'Use one variable with 2 clearly separated conditions and 3 replicates.',details:['1500 rpm — baseline','2000 rpm — comparison','3 replicates per condition · 6 samples total'],confidence:88,evidence:['PSC-2026-041','PSC n-i-p standard v4.1'],warning:'Confirm the spin coater limit and randomise the run order.'}},
  ...
  document.addEventListener('click', event => {
    ...
    if(action==='cabinet-clear-filters'){cabinetCategory='all';if($('#cabinetSearch'))$('#cabinetSearch').value='';if($('#cabinetAvailability'))$('#cabinetAvailability').value='all';if($('#cabinetLocation'))$('#cabinetLocation').value='all';if($('#cabinetFavoritesOnly'))$('#cabinetFavoritesOnly').checked=false;$$('[data-cabinet-filter]').forEach(item=>item.classList.toggle('active',item.dataset.cabinetFilter==='all'));renderCabinet();}
    if(action==='cabinet-open')openCabinetDetail(button.dataset.resourceId);
    if(action==='cabinet-favorite'){const id=button.dataset.resourceId;if(cabinetFavorites.has(id))cabinetFavorites.delete(id);else cabinetFavorites.add(id);$$(`[data-action="cabinet-favorite"][data-resource-id="${id}"]`).forEach(item=>item.classList.toggle('active',cabinetFavorites.has(id)));renderCabinet();}
    if(action==='cabinet-more')toast('Resource actions','info',button.dataset.message||'Duplicate · view uses · archive');
    if(action==='cabinet-use'){const item=cabinetResources.find(resource=>resource.id===button.dataset.resourceId);toast('Resource ready to use','success',`${item?.name||'Resource'} will be linked as an experiment snapshot.`);}
    if(action==='cabinet-quick-create'){const modal=$('#cabinetQuickCreate');if(modal){$('#cabinetCreateName').value='';$('#cabinetDuplicateCheck').innerHTML=`<span>${icon('info')}</span><div><strong>Similar resources will appear here</strong><small>Enter a name to check deterministic demo matches.</small></div>`;openOverlay('cabinetQuickCreate');}else{mountCabinetSelector();cabinetSelectorCategory='all';openOverlay('cabinetSelectorDrawer');renderContextCreate();}}
    if(action==='cabinet-create-confirm'){const name=$('#cabinetCreateName')?.value.trim();if(!name){toast('Resource name required','warning','Add a clear name before creating the record.');return;}const visibility=$('input[name="cabinetVisibility"]:checked')?.value||'shared';cabinetResources.unshift({id:`custom-${Date.now()}`,name,type:$('#cabinetCreateType')?.selectedOptions[0]?.textContent||'Resource',category:$('#cabinetCreateType')?.value==='equipment'?'equipment':$('#cabinetCreateType')?.value==='solution'?'solutions':'materials',status:'Draft',tone:'info',availability:'available',location:$('#cabinetCreateLocation')?.value.trim()||'Location not set',locationKey:'all',uses:1,favorite:false,icon:'NEW',description:$('#cabinetCreateRole')?.value.trim()||'New minimal resource record.',meta:`Created now · ${visibility==='shared'?'shared in Lab Cabinet':'local to experiment'}`,details:[['Visibility',visibility==='shared'?'Shared':'Experiment only'],['Record state','Draft']],usage:'PSC-2026-041 · newly linked resource'});closeOverlay($('#cabinetQuickCreate'));renderCabinet();toast('Resource created and linked','success',visibility==='shared'?'Available in Lab Cabinet and snapshotted in the experiment.':'Kept only in this experiment.');}
    ...
    if(action==='cabinet-select-use'){const item=cabinetResources.find(resource=>resource.id===button.dataset.resourceId);button.textContent='Added';button.disabled=true;button.closest('.cabinet-selector-row')?.classList.add('selected');toast('Added to experiment','success',`${item?.name||'Resource'} is linked with a traceable snapshot.`);}
    if(action==='cabinet-context-create')renderContextCreate();
    if(action==='cabinet-selector-back')renderCabinetSelector();
    if(action==='cabinet-context-confirm'){const name=$('#contextCreateName')?.value.trim();if(!name){toast('Resource name required','warning','Enter a name to create and use this resource.');return;}const visibility=$('input[name="contextVisibility"]:checked')?.value||'shared';const root=$('#cabinetSelectorBody');if(root)root.innerHTML=`<div class="cabinet-create-success"><span>${icon('check')}</span><span class="eyebrow">Resource linked</span><h2>${escapeHtml(name)}</h2><p>${visibility==='shared'?'Shared in Lab Cabinet and copied into this experiment as a snapshot.':'Created only for this experiment.'}</p><div class="row wrap"><button class="button" type="button" data-action="close">Continue experiment</button><button class="button primary" type="button" data-action="toast" data-message="Full resource record opened">Complete record</button></div></div>`;}
    ...
    if(action==='tool-context')toast('Context selector opened','info','Experiment, stack, file, dataset and measure can be changed together.');
    ...
    if(action==='code-copy'){navigator.clipboard?.writeText($('#codeInput')?.value||'');toast('Structured text copied','success','The current working copy is on the clipboard.');}
    ...
    if(action==='nomad-browse'){toast('Browsing NOMAD...','info','Simulating NOMAD repository connection.');const results=$('#nomadImportResults');if(results){setTimeout(()=>{results.innerHTML=`<table class="data-table"><thead><tr><th></th><th>Experiment</th><th>Title</th><th>Date</th><th>Status</th></tr></thead><tbody><tr><td><input type="checkbox"></td><td class="mono">EXP-2025-128</td><td>Lead iodide purity screening</td><td>15 Dec 2025</td><td><span class="badge success">Published</span></td></tr><tr><td><input type="checkbox"></td><td class="mono">EXP-2026-014</td><td>Formamidinium optimisation batch 2</td><td>22 Feb 2026</td><td><span class="badge success">Published</span></td></tr><tr><td><input type="checkbox"></td><td class="mono">EXP-2026-041</td><td>Antisolvent volume screening</td><td>10 Jul 2026</td><td><span class="badge info">Draft</span></td></tr><tr><td><input type="checkbox"></td><td class="mono">EXP-2026-048</td><td>Annealing temperature matrix</td><td>22 Jul 2026</td><td><span class="badge warning">Submitted</span></td></tr></tbody></table><div class="row between" style="margin-top:8px"><span class="subtle">4 experiments found in repository <strong>nomad-lab.eu</strong></span><button class="button primary" type="button" data-action="nomad-import-selected"><span data-icon="download"></span>Import selected</button></div>`;toast('Experiments loaded','success','4 experiments found in NOMAD repository.');},800);}}
    if(action==='nomad-import-selected'){toast('Import started','info','Simulating NOMAD data import. Selected experiments will be mapped to LabFlow format.');setTimeout(()=>toast('Import complete','success','3 experiments imported from NOMAD. Check Projects page.'),1500);}
    ...
    if(action==='experiment-archive'){const archive={manifest:{format:'LabFlow experiment archive',version:'1.3',generated_at:new Date().toISOString(),author:currentUser().name,experiment:'PSC-2026-041',warnings:['2 columns without units','1 instrument association pending']},contents:['experiment.json','stacks/','materials/','solutions/','process/','data/','results/','charts/','reports/','files/','provenance/','nomad-mapping/']};X.download(JSON.stringify(archive,null,2),'application/json','experiment-PSC-2026-041-manifest.json');toast('Archive manifest generated','success','Static POC: the manifest represents the complete ZIP structure and exclusions.');}
  });
  ...
  document.addEventListener('pointerdown',event=>{const user=$('#userPopover'),process=$('#processPopover');if(user&&!user.hidden&&!event.target.closest('#userPopover,.user-menu,[data-action="user-toggle"]'))user.hidden=true;if(process&&!process.hidden&&!event.target.closest('#processPopover,.project-context-button,[data-action="process-toggle"]'))process.hidden=true;});
  ...
  window.addEventListener('hashchange',()=>{const key=location.hash.slice(1);if(page==='experiment'){const step={setup:0,resources:0,solutions:0,stacks:1,processing:2,process:2,data:3,analysis:4,outputs:5,results:5,export:6,nomad:7}[key];if(Number.isInteger(step)&&step!==wizardStep)showWizardStep(step);return;}if(page==='docs'&&documentationOrder.includes(key)){renderDocumentation(key);return;}const tabButton=$(`[data-tab="${key}"]`);if(tabButton)activateTab(tabButton);});

  mountShell(); mountGlobalSearch(); mountAIDrawer(); mountProcessStepWizard(); mountCabinetSelector(); renderCabinet(); renderToolCatalog(); renderKnowledgeSources(); renderKnowledgeWelcome(); if(page==='report')initReportImages(); if(page==='projects'&&location.hash==='#new')setTimeout(openProjectWizard,0);
  ...
  renderEntity(page==='flow'?'UserAccount':'IonDefinition'); renderDocumentation(location.hash.slice(1)||'overview');
